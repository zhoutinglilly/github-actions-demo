...
iCn3DUI.prototype.show3DStructure = function () {
    let me = this
    let thisClass = this
    me.deferred = $.Deferred(function () {
        ...
        $('#' + ic.pre + 'viewer')
            .width(width)
            .height(parseInt(height) + extraHeight)
        $('#' + ic.pre + 'canvas')
            .width(width)
        ...
        if (me.utilsCls.isSessionStorageSupported() && ic.bCrashed) {
            ...
            if (
                ...
                id === me.cfg.mmdbid ||
                id === me.cfg.gi ||
                id === me.cfg.blast_rep_id ||
                id === me.cfg.cid ||
                id === me.cfg.mmcifid ||
                id === me.cfg.align ||
                id === me.cfg.chainalign
            ) {
        ...
        if (me.cfg.url !== undefined) {
            ...
            if (me.cfg.query_id.substr(0, 5) !== 'Query' && me.cfg.rid === undefined) {
                ...
                let url =
                    'https://blast.ncbi.nlm.nih.gov/Blast.cgi?RESULTS_FILE=on&FORMAT_TYPE=JSON2_S&FORMAT_OBJECT=Alignment&CMD=Get&RID=' +
                    me.cfg.rid // e.g., RID=EFTRU3W5014
                $.ajax({
                    url: url,
                    dataType: 'json',
                    tryCount: 0,
            ...
            $.ajax({
                ...
                retryLimit: 1,
                success: function (data) {
                    if (data.InformationList !== undefined && data.InformationList.Information !== undefined)
                        ic.molTitle = data.InformationList.Information[0].Title
                },
                error: function (xhr, textStatus, errorThrown) {
                    this.tryCount++
                    if (this.tryCount <= this.retryLimit) {
...
iCn3DUI.prototype.setIcn3d = function () {
    let me = this
    let str1 =
        "<label class='icn3d-switch'><input id='" +
        me.pre +
        "modeswitch' type='checkbox'><div class='icn3d-slider icn3d-round' style='width:34px; height:18px; margin: 6px 0px 0px 3px;' title='Left(\"All atoms\"): Style and color menu options will be applied to all atoms in the structure&#13;Right(\"Selection\"): Style and color menu options will be applied only to selected atoms'></div></label>"
    let str2 =
        "<span id='" +
        me.pre +
        "modeall' title='Style and color menu options will be applied to all atoms in the structure'>All atoms&nbsp;&nbsp;</span><span id='" +
        me.pre +
        "modeselection' class='icn3d-modeselection' style='display:none;' title='Style and color menu options will be applied only to selected atoms'>Selection&nbsp;&nbsp;</span></div></div></td>"
...
iCn3DUI.prototype.setDialogAjax = function () {
    let me = this
