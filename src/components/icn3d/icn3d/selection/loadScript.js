...
class LoadScript {
    ...
    */
    ...
    execCommandsBase(start, end, steps, bFinalStep) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        let i
        for (i = start; i <= end; ++i) {
            ...
            if (ic.commands[i].indexOf('load') !== -1) {
                ...
                ic.commands[i].trim().indexOf('set map') == 0 &&
                ic.commands[i].trim().indexOf('set map wireframe') == -1
            ) {
                ...
                if (paraArray.length == 3 && paraArray[1] == 'sigma') {
                    ...
                    if (
                        (type == '2fofc' && (ic.bAjax2fofc === undefined || !ic.bAjax2fofc)) ||
                ...
                ic.commands[i].trim().indexOf('set emmap') == 0 &&
                ic.commands[i].trim().indexOf('set emmap wireframe') == -1
            ) {
                ...
                ic.commands[i].trim().indexOf('view annotations') == 0
                ...
                return
            } else if (ic.commands[i].trim().indexOf('set annotation clinvar') == 0) {
                // the command may have "|||{"factor"...
                let strArray = ic.commands[i].split('|||')
                ...
                return
            } else if (ic.commands[i].trim().indexOf('set annotation snp') == 0) {
                // the command may have "|||{"factor"...
                let strArray = ic.commands[i].split('|||')
                ...
                return
            } else if (ic.commands[i].trim().indexOf('set annotation 3ddomain') == 0) {
                // the command may have "|||{"factor"...
                let strArray = ic.commands[i].split('|||')

                if (
                    Object.keys(ic.proteins).length > 0 &&
                    ic.mmdb_data === undefined &&
                    (ic.bAjax3ddomain === undefined || !ic.bAjax3ddomain)
                ) {
                ...
                return
            } else if (ic.commands[i].trim().indexOf('set annotation all') == 0) {
                // the command may have "|||{"factor"...
                let strArray = ic.commands[i].split('|||')
                ...
                if (
                    Object.keys(ic.proteins).length > 0 &&
                    (ic.bAjaxClinvar === undefined || !ic.bAjaxClinvar) &&
                ...
                return
            } else if (ic.commands[i].trim().indexOf('view interactions') == 0 && me.cfg.align !== undefined) {
                // the command may have "|||{"factor"...
                let strArray = ic.commands[i].split('|||')
    ...
    */
    ...
    pressCommandtext() {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        $('#' + ic.pre + 'logtext').keypress(function (e) {
            ...
            if (code == 13) {
                ...
                for (let i = prevLogLen, il = commandArray.length; i < il; ++i) {
                    ...
                    if (lastCommand.indexOf('load') !== -1) {
                        ...
                        lastCommand.indexOf('set map') !== -1 &&
                        lastCommand.indexOf('set map wireframe') === -1
                    ) {
                        thisClass.applyCommandMap(lastCommand)
                    } else if (
                        lastCommand.indexOf('set emmap') !== -1 &&
                        lastCommand.indexOf('set emmap wireframe') === -1
                    ) {
                        ...
                        lastCommand.indexOf('view annotations') == 0
                        ...
                        ic.applyCommandCls.applyCommand(
                            lastCommand + '|||' + ic.transformCls.getTransformationStr(transformation),
                        )
                    }
    ...
    applyCommandMap(command) {
        ...
        // chain functions together
        ic.deferredMap = $.Deferred(function () {
            let ic = thisClass.icn3d
            //"set map 2fofc sigma 1.5"
    ...
    applyCommandEmmap(command) {
        ...
        // chain functions together
        ic.deferredEmmap = $.Deferred(function () {
            let ic = thisClass.icn3d
    ...
    applyCommandSymmetryBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ic.symdCls.retrieveSymmetry(Object.keys(ic.structures)[0])
    }

    applyCommandSymmetry(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    applyCommandRealignBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ic.realignParserCls.realignOnSeqAlign()
    }

    applyCommandRealign(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    applyCommandGraphinteractionBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let paraArray = command.split(' | ')
        if (paraArray.length >= 3) {
            ...
            ic.viewInterPairsCls.viewInteractionPairs(
                nameArray2,
                nameArray,
                bHbondCalc,
                'graph',
    ...
    applyCommandGraphinteraction(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    applyCommandCartoon2dBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    applyCommandAnnotationsAndCddSiteBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // chain functions together
    ...
    applyCommandAnnotationsAndCddSite(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    applyCommandClinvarBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // chain functions together
    ...
    applyCommand3ddomainBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // chain functions together
    ...
    applyCommandViewinteractionBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // chain functions together
    ...
    applyCommandViewinteraction(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        // chain functions together
    ...
    renderFinalStep(steps) {
        ...
        if (
            ...
            (ic.optsHistory[steps - 1] !== undefined &&
                ic.optsHistory[steps - 1].hasOwnProperty('hlatomcount') &&
                ic.optsHistory[steps - 1].hlatomcount === Object.keys(ic.atoms).length)
        ) {
            ...
            // more complicated if partial atoms are modified
            ic.hlUpdateCls.updateHlAll()
        ...
        if (me.deferred !== undefined) me.deferred.resolve()
        if (ic.deferred2 !== undefined) ic.deferred2.resolve()
    }

    replayFirstStep(currentNumber) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // fresh start
        ic.reinitAfterLoad()
