...
class ApplyCommand {
    ...
    applyCommand(commandStr) {
        ...
        if (command == 'share link') {
            ...
            // last step to update transformation
        ...
        // start with =================
            ...
            ic.analysisCls.addLine(
                parseFloat(p1Array[1]),
                parseFloat(p1Array[3]),
                parseFloat(p1Array[5]),
                parseFloat(p2Array[1]),
                parseFloat(p2Array[3]),
                parseFloat(p2Array[5]),
                color,
                dashed,
                type,
            )
            ...
            for (let i = 0, il = pos_from_toArray.length; i < il; ++i) {
                ...
                ic.targetGapHash[parseInt(pos_from_to[0])] = {
                    from: parseInt(pos_from_to[1]),
                    to: parseInt(pos_from_to[2]),
                }
            }
            ...
            commandOri.indexOf('display interaction 3d') == 0 ||
        ...
        // special, select ==========
            ...
            if (select.indexOf(' or ') !== -1) {
                // "select " command without " | name"
                ic.selByCommCls.selectByCommand(select, commandname, commanddesc)
            } else {
                // only single query from selectByCommand()
    ...
    getThresholdNameArrays(commandOri) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        let nameArray = [],
            nameArray2 = []
        if (paraArray.length >= 2 && paraArray[1].length > 4) {
            //sets a,b,c e,f,g
    ...
    getMenuFromCmd(cmd) {
        ...
        else if (cmd.indexOf('set map') == 0 && cmd.indexOf('set map wireframe') == -1)
            return 'Style > Electron Density'
        else if (cmd.indexOf('set emmap') == 0 && cmd.indexOf('set emmap wireframe') == -1)
            return 'Style > EM Density Map'
        ...
        //else if(cmd == 'set surface neighbors off') return 'Style > Surface Type > ... without Context';
            ...
            cmd == 'set hbonds off' ||
            cmd == 'set salt bridge off' ||
            cmd == 'set contact off' ||
            cmd == 'set halogen pi off'
        )
            return hbondIntStr + ' > Reset'
        ...
        else if (cmd.indexOf('select saved atoms') == 0 || cmd.indexOf('select sets') == 0)
            return 'Windows > Defined Sets: select in menu'
        else if (cmd.indexOf('select chain') !== -1) return seqAnnoStr + ': click on chain names'
        else if (cmd.indexOf('select alignChain') !== -1)
            return 'Windows > View Aligned Sequences: click on chain names'
        ...
        else if (cmd.indexOf('view interaction pairs') == 0)
            return hbondIntStr + ': "Highlight Interactions in Table" button'
        ...
        else if (cmd.indexOf('line graph interaction pairs') == 0)
            return hbondIntStr + ': "2D Interaction Network" button'
        ...
        else if (cmd.indexOf('custom tube') == 0)
            return seqAnnoStr + ': "Custom Color/Tube" button: "Custom Tube" button'
        ...
        else if (cmd.indexOf('select') == 0 && cmd.indexOf('name') !== -1)
            return seqAnnoStr + ': drag on residues to select'
        else if (
            cmd.indexOf('select $') !== -1 ||
            cmd.indexOf('select .') !== -1 ||
            cmd.indexOf('select :') !== -1 ||
            cmd.indexOf('select @') !== -1
        )
            return 'Select > Advanced; or other selection'
