...
class DefinedSets {
    ...
    setPredefinedInMenu() {
        ...
        if (
            (me.cfg.align !== undefined || (me.cfg.chainalign !== undefined && ic.chainidArray.length == 2)) &&
            ic.bFullUi
        ) {
    ...
    setAtomMenu(commandnameArray) {
        ...
        for (let i = 0, il = nameArray.length; i < il; ++i) {
            ...
            let colorStr =
                atom === undefined || atom.color === undefined || atom.color.getHexString().toUpperCase() === 'FFFFFF'
                    ? 'DDDDDD'
    ...
    clickCommand_apply() {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        me.myEventCls.onIds('#' + ic.pre + 'command_apply', 'click', function (e) {
            let ic = thisClass.icn3d
            e.preventDefault()
        ...
        me.myEventCls.onIds('#' + ic.pre + 'command_apply2', 'click', function (e) {
            let ic = thisClass.icn3d
            e.preventDefault()
        ...
        for (let i = 0, il = nameArray.length; i < il; ++i) {
            if (nameArray[i] === 'non-selected') {
                ...
                selAtoms = me.hashUtilsCls.unionHash(
                    selAtoms,
                    me.hashUtilsCls.hash2Atoms(this.getAtomsFromOneSet(nameArray[i]), ic.atoms),
                )
            }
        }
