...
class Selection {
    ...
    selectAChain(chainid, commandname, bAlign, bUnion) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        let oriResidueHash = {}
        for (let i = 0, il = chnsSeqLen; i < il; ++i) {
            // get residue number
        ...
        if (
            (ic.defNames2Atoms === undefined || !ic.defNames2Atoms.hasOwnProperty(commandname)) &&
            (ic.defNames2Residues === undefined || !ic.defNames2Residues.hasOwnProperty(commandname))
        ) {
    ...
    selectResidueList(residueHash, commandname, commanddescr, bUnion, bUpdateHighlight, bAtom) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (residueHash !== undefined && Object.keys(residueHash).length > 0) {
            ...
            if (bUpdateHighlight === undefined || bUpdateHighlight)
                ic.hlUpdateCls.updateHlAll(Object.keys(ic.menuHlHash), undefined, bUnion)
        }
    }
    ...
    clickShow_selected() {
        ...
        let thisClass = this
        me.myEventCls.onIds(
            ['#' + ic.pre + 'show_selected', '#' + ic.pre + 'mn2_show_selected'],
            'click',
            function (e) {
    ...
    clickHide_selected() {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        me.myEventCls.onIds('#' + ic.pre + 'mn2_hide_selected', 'click', function (e) {
            let ic = thisClass.icn3d
    ...
    addCustomSelection(residueAtomArray, commandname, commanddesc, select, bSelectResidues) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (bSelectResidues) {
    ...
    saveSelection(name, description) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ic.selectedResidues = {}
        ...
        if (Object.keys(ic.selectedResidues).length > 0) {
            if (ic.pk == 1) {
                ...
                me.htmlCls.clickMenuCls.setLogCmd(
                    'select ' + ic.resid2specCls.atoms2spec(ic.hAtoms) + ' | name ' + name,
                    true,
                )
            } else {
                ...
                me.htmlCls.clickMenuCls.setLogCmd(
                    'select ' + ic.resid2specCls.residueids2spec(Object.keys(ic.selectedResidues)) + ' | name ' + name,
                    true,
                )
            }
        }
    ...
    saveSelectionIfSelected(id, value) {
        var ic = this.icn3d,
            me = ic.icn3dui
        if (ic.bSelectResidue || ic.bSelectAlignResidue) {
            let name = $('#' + ic.pre + 'seq_command_name2')
                .val()
                .replace(/\s+/g, '_')
            //var description = $("#" + ic.pre + "seq_command_desc2").val();
            if (name === '') {
                name = $('#' + ic.pre + 'alignseq_command_name')
                    .val()
                    .replace(/\s+/g, '_')
    ...
    selectOneResid(idStr, bUnchecked) {
        var ic = this.icn3d,
            me = ic.icn3dui
    ...
    adjustMembrane(extra_mem_z, intra_mem_z) {
        var ic = this.icn3d,
            me = ic.icn3dui
