...
class Resid2spec {
    ...
    selectProperty(property, from, to) {
        var ic = this.icn3d,
            me = ic.icn3dui
        ...
        if (property == 'positive') {
            ...
            for (let i in atoms) {
                ...
                if (atom.b >= from && atom.b <= to) {
                    ic.hAtoms = me.hashUtilsCls.unionHash(
                        ic.hAtoms,
                        ic.residues[atom.structure + '_' + atom.chain + '_' + atom.resi],
                    )
                }
            }
            ...
            for (let resid in ic.resid2area) {
                // resid: structure_chain_resi_resn
    ...
    switchHighlightLevel() {
        ...
        document.addEventListener('keydown', function (e) {
            ...
            if (e.keyCode === 38) {
                // arrow up, select upper level of atoms
                e.preventDefault()
                if (
                    Object.keys(ic.pickedAtomList).length == 0 ||
                    !ic.hAtoms.hasOwnProperty(ic.firstAtomObjCls.getFirstAtomObj(ic.pickedAtomList).serial)
                ) {
                ...
                // arrow down, select down level of atoms
                e.preventDefault()
                if (
                    Object.keys(ic.pickedAtomList).length == 0 ||
                    !ic.hAtoms.hasOwnProperty(ic.firstAtomObjCls.getFirstAtomObj(ic.pickedAtomList).serial)
    ...
    switchHighlightLevelUp() {
        ...
        if (ic.highlightlevel === 1) {
            // atom -> residue
            ic.highlightlevel = 2
            let firstAtom = ic.firstAtomObjCls.getFirstAtomObj(ic.pickedAtomList)
            if (!ic.bShift && !ic.bCtrl) {
                ic.hAtoms = me.hashUtilsCls.cloneHash(
                    ic.residues[firstAtom.structure + '_' + firstAtom.chain + '_' + firstAtom.resi],
                )
            } else {
                ic.hAtoms = me.hashUtilsCls.unionHash(
                    ic.hAtoms,
            ...
            if ((me.cfg.mmdbid === undefined && me.cfg.gi === undefined) || Object.keys(atomLevel4).length == 0) {
                // strand -> chain
                ic.highlightlevel = 5
                let firstAtom = ic.firstAtomObjCls.getFirstAtomObj(ic.pickedAtomList)
                if (!ic.bShift && !ic.bCtrl) {
                    ...
                    ic.hAtoms = me.hashUtilsCls.unionHash(
                        ic.hAtoms,
                        ic.chains[firstAtom.structure + '_' + firstAtom.chain],
                    )
                }
            }
        } else if (ic.highlightlevel === 4) {
            // domain -> chain
            ic.highlightlevel = 5
            let firstAtom = ic.firstAtomObjCls.getFirstAtomObj(ic.pickedAtomList)
            if (!ic.bShift && !ic.bCtrl) {
        ...
        } else if (ic.highlightlevel === 5 || ic.highlightlevel === 6) {
            // chain -> structure
            ic.highlightlevel = 6
            let firstAtom = ic.firstAtomObjCls.getFirstAtomObj(ic.pickedAtomList)
    ...
    switchHighlightLevelDown() {
        ...
        if ((ic.highlightlevel === 2 || ic.highlightlevel === 1) && Object.keys(ic.pickedAtomList).length === 1) {
            // residue -> atom
            ic.highlightlevel = 1
