...
class AnnoCddSite {
    ...
    setDomainFeature(domainArray, chnid, bDomain, html, html2, html3, acc2domain) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        ...
        for (let index = 0; index < indexl; ++index) {
            ...
            for (let r = 0, rl = domainRepeatArray.length; r < rl; ++r) {
                ...
                for (let s = 0, sl = segArray.length; s < sl; ++s) {
                    ...
                    fromArray.push(
                        thisClass.getAdjustedResi(domainFrom, chnid, ic.matchedPos, ic.chainsSeq, ic.baseResi) - 1,
                    )
                    toArray.push(
                        thisClass.getAdjustedResi(domainTo, chnid, ic.matchedPos, ic.chainsSeq, ic.baseResi) - 1,
                    )
                ...
                let htmlTmp2 =
                    '<div class="icn3d-seqTitle icn3d-link icn3d-blue" ' +
                    type +
                    '="' +
                    acc +
                    '" from="' +
                ...
                if (bDomain) {
                    html2 +=
                        '<div style="width:20px; display:inline-block;"><span id="' +
                        ic.pre +
                        chnid +
                        '_' +
                        acc +
                        '_' +
                        r +
                        '_cddseq_expand" class="ui-icon ui-icon-plus icn3d-expand icn3d-link" style="width:15px;" title="Expand"></span><span id="' +
                        ic.pre +
                        chnid +
                        '_' +
                        acc +
                        '_' +
                        r +
                        '_cddseq_shrink" class="ui-icon ui-icon-minus icn3d-shrink icn3d-link" style="display:none; width:15px;" title="Shrink"></span></div>'
                }
                html2 +=
                    '<div style="width:' +
                    titleSpace +
                    'px!important;" class="icn3d-seqTitle icn3d-link icn3d-blue" ' +
                    type +
                    '="' +
                    acc +
                    '" from="' +
                ...
                if (me.cfg.blast_rep_id != chnid) {
                    ...
                    for (let i = 0, il = fromArray.length; i < il; ++i) {
                        let emptyWidth =
                            i == 0
                                ? Math.round(
                                      (ic.seqAnnWidth * (fromArray[i] - ic.baseResi[chnid] - 1)) / ic.maxAnnoLength,
                                  )
                        ...
                        html2 +=
                            ...
                            color +
                            '; width:' +
                            Math.round((ic.seqAnnWidth * (toArray[i] - fromArray[i] + 1)) / ic.maxAnnoLength) +
                            'px;" class="icn3d-seqTitle icn3d-link icn3d-blue" domain="' +
                            (index + 1).toString() +
                            '" from="' +
                            fromArray +
                            '" to="' +
                    ...
                    for (let i = 0, il = fromArray2.length; i < il; ++i) {
                        ...
                        html2 +=
                            ...
                            '; width:' +
                            Math.round(
                                (ic.seqAnnWidth * (toArray2[i] - fromArray2[i] + 1)) /
                                    (ic.maxAnnoLength + ic.nTotalGap),
                            ) +
                            'px;" class="icn3d-seqTitle icn3d-link icn3d-blue" domain="' +
                            (index + 1).toString() +
                ...
                if (bDomain) {
                    html2 +=
                        ...
                        '_' +
                        r +
                        '_cddseq" style="display:none; white-space:normal;" class="icn3d-box">' +
                        defline +
                        '(<a href="' +
                        me.htmlCls.baseUrl +
                        'cdd/cddsrv.cgi?uid=' +
                        acc +
                        '" target="_blank" class="icn3d-blue">open details view...</a>)</div>'
                }
            } // for(let r = 0,
        }
        ...
        return {
            ...
            html2: html2,
            html3: html3,
            acc2domain: acc2domain,
            pssmid2name: pssmid2name,
            pssmid2fromArray: pssmid2fromArray,
            pssmid2toArray: pssmid2toArray,
        }
    }

    getAdjustedResi(resi, chnid, matchedPos, chainsSeq, baseResi) {
        let ic = this.icn3d,
            me = ic.icn3dui
        return resi >= matchedPos[chnid] && resi - matchedPos[chnid] < ic.chainsSeq[chnid].length
            ? ic.chainsSeq[chnid][resi - matchedPos[chnid]].resi
            : baseResi[chnid] + 1 + resi
    }

    showAnnoType(chnid, chnidBase, type, title, residueArray, resid2resids) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    setToolTip() {
        let ic = this.icn3d,
            me = ic.icn3dui
        $('[id^=' + ic.pre + 'snp]')
            .add('[id^=' + ic.pre + 'clinvar]')
            .add('[id^=' + ic.pre + 'ssbond]')
            .add('[id^=' + ic.pre + 'crosslink]')
            .tooltip({
                content: function () {
