...
class AnnoSnpClinVar {
    ...
    navClinVar(chnid) {
        ...
        let thisClass = this
        ic.currClin[chnid] = -1
        me.myEventCls.onIds('#' + ic.pre + chnid + '_prevclin', 'click', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            //e.preventDefault();
            let maxLen =
                ic.resi2disease_nonempty[chnid] !== undefined ? Object.keys(ic.resi2disease_nonempty[chnid]).length : 0
            --ic.currClin[chnid]
            if (ic.currClin[chnid] < 0) ic.currClin[chnid] = maxLen - 1 // 0;
            thisClass.showClinVarLabelOn3D(chnid)
        })
        me.myEventCls.onIds('#' + ic.pre + chnid + '_nextclin', 'click', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            //e.preventDefault();
            let maxLen =
    ...
    getSnpLine(
        ...
        if (bStartEndRes) {
            ...
            if (!bVirus && ic.organism !== undefined && ic.organism !== 'human' && ic.organism !== 'homo sapiens') {
                ...
                warning2 =
                    " <span style='color:#FFA500'>(based on human sequences and mapped to this structure by sequence similarity)</span>"
            }
            if (bClinvar) {
                html +=
                    '<div class="icn3d-seqTitle icn3d-link icn3d-blue icn3d-clinvar-path" clinvar="clinvar" posarray="' +
                    posClinArray +
                    '" shorttitle="' +
                    title1 +
                    '" setname="' +
                ...
                html +=
                    '<div class="icn3d-seqTitle icn3d-link icn3d-blue" clinvar="clinvar" posarray="' +
                    posarray +
                    '" shorttitle="' +
                    title2 +
                    '" setname="' +
            ...
            html +=
                ...
                ic.pre +
                chnid +
                '_prevclin" style="display:inline-block; font-size:11px; font-weight:bold; width:60px!important;"><button class="link" style="-webkit-appearance:' +
                buttonStyle +
                '; height:18px; width:55px;"><span style="white-space:nowrap; margin-left:-40px;" title="Show the previous ClinVar on structure">&lt; ClinVar</span></button></div>'
            html +=
                '<div id="' +
                ic.pre +
                chnid +
                '_nextclin" style="display:inline-block; font-size:11px; font-weight:bold; width:60px!important;"><button class="link" style="-webkit-appearance:' +
                buttonStyle +
                '; height:18px; width:55px;"><span style="white-space:nowrap; margin-left:-40px;" title="Show the next ClinVar on structure">ClinVar &gt;</span></button></div>'
        } else {
            ...
            if (resi2index[i] !== undefined) {
                ...
                for (let j = 0, jl = resi2snp[i].length; j < jl && !bSnpOnly; ++j) {
                    ...
                    for (let k = 0, kl = diseaseArray.length; k < kl; ++k) {
                        if (
                            diseaseArray[k] != '' &&
                            diseaseArray[k] != 'not specified' &&
                            diseaseArray[k] != 'not provided'
                        ) {
                    ...
                    if (diseaseTitle != '') {
                        ...
                        if (j == line - 2) {
                            // just check the current line, "line = 2" means the first SNP
        ...
        for (let i = 1, il = ic.giSeq[chnid].length; i <= il; ++i) {
            if (bOverview) {
                if (resi2index[i] !== undefined) {
                    ...
                    let pos =
                        i >= ic.matchedPos[chnid] && i - 1 - ic.matchedPos[chnid] < ic.chainsSeq[chnid].length
                            ? ic.chainsSeq[chnid][i - 1 - ic.matchedPos[chnid]].resi
                            : ic.baseResi[chnid] + 1 + i - 1
                    let snpTitle = pos + c + '>'
                    let allDiseaseTitle = ''
                    ...
                    let emptyWidth =
                        me.cfg.blast_rep_id == chnid
                            ? Math.round(
                                  (ic.seqAnnWidth * (i - 1)) / (ic.maxAnnoLength + ic.nTotalGap) -
                                      prevEmptyWidth -
                                      prevLineWidth,
                              )
                            : Math.round((ic.seqAnnWidth * (i - 1)) / ic.maxAnnoLength - prevEmptyWidth - prevLineWidth)
                            ...
                            for (let j = start; j < jl && j < end; ++j) {
                                ...
                                if (!bSnpOnly) {
                                    ...
                                    if (diseaseTitle != '') {
                                        ...
                                        if (bCoord && !me.cfg.hidelicense) {
                                            ...
                                            snpTitle +=
                                                ic.showAnnoCls.addSnpButton(
                                                    ...
                                                    'snpinter',
                                                    'Interactions',
                                                    'SNP Interactions in 3D',
                                                    70,
                                                    buttonStyle,
                                                ) + '&nbsp;&nbsp;'
                                            snpTitle += ic.showAnnoCls.addSnpButton(
                                                snpTmpStr,
                                                'snppdb',
                                                'PDB',
                                                'Download SNP PDB',
                                        ...
                                        snpTitle +=
                                            "<br>Links: <a href='https://www.ncbi.nlm.nih.gov/clinvar/?term=" +
                                            resi2clinAllele[i][j] +
                                            "[AlleleID]' target='_blank'>ClinVar</a>, <a href='https://www.ncbi.nlm.nih.gov/snp/?term=" +
                                            resi2rsnum[i][j] +
                                            "' target='_blank'>dbSNP(rs" +
                                            resi2rsnum[i][j] +
                                            ')</a>'
                                    } else {
                                        if (bCoord && !me.cfg.hidelicense) {
                                        ...
                                        //snpTitle += "<br>Links: <span class='" + ic.pre + "snpin3d icn3d-snplink' snp='" + chnid + "_" + pos + "_" + resi2snp[i][j] + "'>SNP in 3D with scap</span>, <span class='" + ic.pre + "snpinter icn3d-snplink' snp='" + chnid + "_" + pos + "_" + resi2snp[i][j] + "'>SNP Interactions in 3D</span>, <span class='" + ic.pre + "snppdb icn3d-snplink' snp='" + chnid + "_" + pos + "_" + resi2snp[i][j] + "'>SNP PDB</span>, <a href='https://www.ncbi.nlm.nih.gov/snp/?term=" + resi2rsnum[i][j] + "' target='_blank'>dbSNP(rs" + resi2rsnum[i][j] + ")</a>"
                                        snpTitle +=
                                            "<br>Link: <a href='https://www.ncbi.nlm.nih.gov/snp/?term=" +
                                            resi2rsnum[i][j] +
                                            "' target='_blank'>dbSNP(rs" +
                                            resi2rsnum[i][j] +
                                            ')</a>'
                                ...
                                if (line == 1) {
                                    ...
                                    if (snpStr == '' || snpStr == ' ') {
                                        ...
                                        html +=
                                            ...
                                            '" ' +
                                            diseaseStr +
                                            ' class="icn3d-tooltip icn3d-residue ' +
                                            currSnpTypeHash[i] +
                                            '">' +
                                            snpStr +
                                            '</span>'
    ...
    processSnpClinvar(data, chnid, chnidBase, bSnpOnly, bVirus) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    showClinvarPart2(chnid, chnidBase, gi) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    showSnp(chnid, chnidBase) {
        let ic = this.icn3d,
            me = ic.icn3dui
        this.showSnpClinvar(chnid, chnidBase, true)
    }
    showClinvar(chnid, chnidBase) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    showSnpClinvar(chnid, chnidBase, bSnpOnly) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    showSnpPart2(chnid, chnidBase, gi) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        if (gi !== undefined) {
            let url3 =
                'https://www.ncbi.nlm.nih.gov/projects/SNP/beVarSearch.cgi?appname=iCn3D&format=bed&report=pdb2bed&connect=MSSNPSUBMISSION1&gi=' +
                gi
