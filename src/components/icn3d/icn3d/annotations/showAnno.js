...
class ShowAnno {
    ...
    showAnnotations() {
        ...
        if ((ic.bAssemblyNote === undefined || !ic.bAssemblyNote) && ic.asuCnt !== undefined) {
            let html =
                "     <br><div id='" +
                ic.pre +
                "assembly_note' style='margin-left:5px;'><span class='icn3d-annoLargeTitle'>Assembly Tips:</span> Only the asymmetric unit is shown in the sequence window.<br>Click \"Assembly\" in the menu \"View\" to switch between asymmetric unit and biological assembly(<b>" +
                ic.asuCnt +
                '</b> asymmetric unit).</div>'
        ...
        if (ic.bAnnoShown === undefined || !ic.bAnnoShown) {
            ...
            let dialogWidth = me.cfg.notebook
                ? me.htmlCls.WIDTH / 2
                : $('#' + ic.pre + 'dl_selectannotations').dialog('option', 'width')
            ...
            for (let i = 0, il = chainArray.length; i < il; ++i) {
                ...
                let chainidBase
                if (chainLetter.indexOf('_') !== -1) {
                    // NCBI modified chainid, e.g., A_1
            ...
            if (me.cfg.blast_rep_id === undefined) {
                if (ic.bFullUi) {
                    if (me.cfg.mmtfid !== undefined) {
                        // mmtf data do NOT have the missing residues
    ...
    showAnnoSeqData(nucleotide_chainid, chemical_chainid, chemical_set) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    getAnnotationData() {
        ...
        for (let chnid in ic.protein_chainid) {
            ...
            let geneLink =
                ic.chainsGene[chnid] && ic.chainsGene[chnid].geneId
                    ? "(Gene: <a href='https://www.ncbi.nlm.nih.gov/gene/" +
                      ic.chainsGene[chnid].geneId +
                      "' target='_blank' title='" +
                      ic.chainsGene[chnid].geneDesc +
                      "'>" +
                      ic.chainsGene[chnid].geneSymbol +
                      '</a>)'
                    : ''
            let chainHtml =
                "<div id='" +
            ...
            chainHtml +=
                this.addButton(
                    ...
                    'icn3d-customcolor',
                    'Custom Color/Tube',
                    'Use a custom file to define the colors or tubes in 3D structure',
                    110,
                    buttonStyle,
                ) + '&nbsp;&nbsp;&nbsp;'
            //}
            chainHtml +=
                this.addButton(
                    ...
                    'icn3d-helixsets',
                    'Helix Sets',
                    'Define sets for each helix in this chain and add them to the menu of "Defined Sets"',
                    60,
                    buttonStyle,
                ) +
                '&nbsp;' +
                this.addButton(
                    ...
                    'icn3d-sheetsets',
                    'Sheet Sets',
                    'Define sets for each sheet in this chain and add them to the menu of "Defined Sets"',
                    60,
                    buttonStyle,
                ) +
                '&nbsp;' +
                this.addButton(
                    ...
                    'icn3d-coilsets',
                    'Coil Sets',
                    'Define sets for each coil in this chain and add them to the menu of "Defined Sets"',
                    60,
                    buttonStyle,
                )
    ...
    getSequenceData(chnid, chnidBase, type, index) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let fullProteinName = ic.showSeqCls.getProteinName(chnid)
        let proteinName = fullProteinName
    ...
    getCombinedSequenceData(name, residArray, index) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let categoryStr =
            index == 0 ? "<span class='icn3d-annoLargeTitle'><b>Chemicals/Ions/Water</b>: </span><br><br>" : ''
        let chemName
        ...
        if (sid !== undefined) {
            chemName =
                "<b><a class='icn3d-blue' href='https://pubchem.ncbi.nlm.nih.gov/substance/" +
                sid +
                "#section=2D-Structure' target='_blank'>" +
                name +
                " <img src='https://pubchem.ncbi.nlm.nih.gov/image/imgsrv.fcgi?sid=" +
                sid +
                "'></a></b>"
        } else {
            chemName = '<b>' + name + '</b>'
        ...
        $('#' + ic.pre + 'dl_annotations').append(
            "<div id='" + ic.pre + 'anno_' + name + "' class='icn3d-annotation'>" + categoryStr + chemName + '</div>',
        )
        // dt: detailed view, hide by default; ov: overview, show by default
        ...
        htmlTmp +=
            '<div class="icn3d-seqTitle2" anno="sequence"><span style="white-space:nowrap;" title="' +
            chainTypeFull +
            ' ' +
            name +
            '">' +
            ...
            '</span></div>'
        htmlTmp +=
            '<span class="icn3d-residueNum" style="width:60px!important;" title="starting protein sequence number">Count: ' +
            residArray.length +
            '</span>'
        ...
        for (let i = 0, il = residArray.length; i < il; ++i) {
            ...
            html +=
                ...
                cFull +
                resi +
                '" class="icn3d-residue icn3d-chemical">' +
                c +
                '</span>'
        }
        ...
        if (width < 1) width = 1
        html2 +=
            '<div class="icn3d-seqTitle" style="display:inline-block; color:white; font-weight:bold; background-color:' +
            color +
            '; width:' +
            width +
            'px;">&nbsp;</div>'
    ...
    processSeqData(chainid_seq) {
        ...
        for (let chnid in ic.protein_chainid) {
            ...
            if (chainid_seq.hasOwnProperty(chnidBase)) {
                ...
                if (pos == -1) {
                    console.log(
                        "The gi sequence didn't match the protein sequence. The start of 3D protein sequence: " +
                            startResStr +
                            '. The gi sequence: ' +
                            allSeq.substr(0, 10) +
                            '.',
            ...
            if (me.cfg.blast_rep_id != chnid) {
                ...
                if (query !== undefined && target !== undefined) {
                    ...
                    for (let i = 0, il = segArray.length; i < il; ++i) {
                        ...
                        if (i > 0) {
                            // determine gap
                            if (seg.orifrom - prevTargetTo < seg.from - prevQueryTo) {
                                // gap in target
                                ic.targetGapHash[seg.orifrom] = {from: prevQueryTo + 1, to: seg.from - 1}
                                ic.nTotalGap +=
                                    ic.targetGapHash[seg.orifrom].to - ic.targetGapHash[seg.orifrom].from + 1
                            } else if (seg.orifrom - prevTargetTo > seg.from - prevQueryTo) {
                                // gap in query
                    ...
                    for (let i = 0, il = targetSeq.length; i < il; ++i) {
                        ...
                        if (ic.targetGapHash.hasOwnProperty(i))
                            nGap += ic.targetGapHash[i].to - ic.targetGapHash[i].from + 1
                        ...
                        if (target2queryHash.hasOwnProperty(i) && target2queryHash[i] !== -1) {
                            ...
                            if (targetSeq[i] == querySeq[target2queryHash[i]]) {
                                ...
                                ic.fullpos2ConsTargetpos[i + nGap] = {
                                    same: 1,
                                    pos: pos,
                                    res: targetSeq[i],
                                    color: colorHexStr,
                                ...
                                ic.alnChainsSeq[chnid].push({
                                    resi: pos,
                                    color: me.htmlCls.GREYC,
                                    color2: '#' + colorHexStr,
                                })
                            }
                        } else {
    ...
    getAnDiv(chnid, anno) {
        ...
        return (
            ...
            '_' +
            chnid +
            "' class='icn3d-fixed-pos' style='display:none!important'></div><div id='" +
            ic.pre +
            'dt_' +
            anno +
            '_' +
    ...
    addButton(chnid, classvalue, name, desc, width, buttonStyle) {
        let ic = this.icn3d,
            me = ic.icn3dui
        return (
            "<div class='" +
            ...
            "' chainid='" +
            chnid +
            "' style='display:inline-block; font-size:11px; font-weight:bold; width:" +
            width +
            "px!important;'><button style='-webkit-appearance:" +
            buttonStyle +
            '; height:18px; width:' +
            width +
            "px;'><span style='white-space:nowrap; margin-left:-3px;' title='" +
            desc +
            "'>" +
            name +
            '</span></button></div>'
    ...
    addSnpButton(snp, classvalue, name, desc, width, buttonStyle) {
        let ic = this.icn3d,
            me = ic.icn3dui
        return (
            "<div class='" +
            ...
            "' snp='" +
            snp +
            "' style='margin:3px 0 3px 0; display:inline-block; font-size:11px; font-weight:bold; width:" +
            width +
            "px!important;'><button style='-webkit-appearance:" +
            buttonStyle +
            '; height:18px; width:' +
    ...
    conservativeReplacement(resA, resB) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let iA =
            me.parasCls.b62ResArray.indexOf(resA) !== -1
                ? me.parasCls.b62ResArray.indexOf(resA)
                : me.parasCls.b62ResArray.length - 1 // or the last one "*"
        let iB =
            me.parasCls.b62ResArray.indexOf(resB) !== -1
                ? me.parasCls.b62ResArray.indexOf(resB)
                : me.parasCls.b62ResArray.length - 1 // or the last one "*"
    ...
    getColorhexFromBlosum62(resA, resB) {
        let ic = this.icn3d,
            me = ic.icn3dui
