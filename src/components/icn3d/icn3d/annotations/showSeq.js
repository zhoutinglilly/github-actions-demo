...
class ShowSeq {
    ...
    showSeq(chnid, chnidBase, type, queryTitle, compTitle, queryText, compText) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let bNonMmdb = false
        let giSeq
        ...
        if (ic.giSeq[chnid].length > 10) {
            ...
            if (
                (me.cfg.mmdbid !== undefined ||
                    ...
                    me.cfg.align !== undefined ||
                    me.cfg.chainalign !== undefined) &&
                atom.resi_ori !== undefined &&
                atom.resi_ori != atom.resi &&
                chnid.indexOf('Misc') == -1
            ) {
                htmlTmp +=
                    '<div class="icn3d-annoTitle" anno="0" title="NCBI Residue Numbers">NCBI Residue Numbers</div>'
            } else {
            ...
            for (let i = 0, il = giSeq.length; i < il; ++i) {
                ...
                if (bNonMmdb) {
                    ...
                    currResi =
                        i >= ic.matchedPos[chnid] && i - ic.matchedPos[chnid] < ic.chainsSeq[chnid].length
                            ? ic.chainsSeq[chnid][i - ic.matchedPos[chnid]].resi
                            : ic.baseResi[chnid] + 1 + i
                }
                html += '<span>'
                if (currResi % 10 === 0) {
                    //html += currResi + ' ';
        ...
        if (me.cfg.blast_rep_id === chnid) {
            htmlTmp =
                '<div id="' + ic.pre + 'giseq_sequence" class="icn3d-dl_sequence" style="border: solid 1px #000;">'
        } else {
            htmlTmp = '<div id="' + ic.pre + 'giseq_sequence" class="icn3d-dl_sequence">'
        }
        let chainType = 'Protein',
            chainTypeFull = 'Protein'
        if (type !== undefined) {
            if (type == 'nucleotide') {
                chainType = 'Nucl.'
        ...
        htmlTmp +=
            '<div class="icn3d-seqTitle icn3d-link icn3d-blue" gi="' +
            chnid +
            '" anno="sequence" chain="' +
            chnid +
            ...
            '</span></div>'
        htmlTmp +=
            '<span class="icn3d-residueNum" title="starting protein sequence number">' +
            (ic.baseResi[chnid] + 1).toString() +
            '</span>'
        html3 += htmlTmp + '<br>'
        ...
        for (let i = 0, il = giSeq.length; i < il; ++i) {
            ...
            if (!ic.residues.hasOwnProperty(chnid + '_' + pos)) {
                ...
                if (
                    ...
                    ic.fullpos2ConsTargetpos !== undefined &&
                    ic.fullpos2ConsTargetpos[i + nGap] !== undefined
                ) {
                ...
                html +=
                    ...
                    cFull +
                    pos +
                    '" class="icn3d-residue" style="color:#' +
                    color +
                    '">' +
                    c +
                    '</span>'
        ...
        if (me.cfg.blast_rep_id != chnid) {
            // regular
            html2 +=
                '<div id="giseq_summary_' +
                ic.pre +
                chnid +
                '" class="icn3d-seqTitle icn3d-link" gi chain="' +
                chnid +
                '" style="display:inline-block; color:white; font-weight:bold; background-color:#' +
                color +
                '; width:' +
                width +
                'px;">' +
            ...
            for (let i = 0, il = fromArray2.length; i < il; ++i) {
                ...
                html2 +=
                    ...
                    'px;" class="icn3d-seqTitle icn3d-link icn3d-blue" anno="sequence" gi chain="' +
                    chnid +
                    '" title="' +
                    chnid +
                    '">' +
        ...
        if (me.cfg.blast_rep_id == chnid) {
            ...
            if (compText !== undefined && compText !== '') {
                ...
                htmlTmp =
                    '<div class="icn3d-seqTitle icn3d-link icn3d-blue" blast="" posarray="' +
                    ic.consrvResPosArray.toString() +
                    '" title="' +
                    compTitle +
                    '" setname="' +
                    chnid +
                    '_blast" anno="sequence" chain="' +
                    chnid +
                    '"><span style="white-space:nowrap;" title="' +
                    compTitle +
                    '">' +
                    ...
                    if (c == '-') {
                        ...
                        if (!ic.residues.hasOwnProperty(chnid + '_' + pos)) {
                            ...
                            html +=
                                ...
                                pos +
                                '" title="' +
                                ic.fullpos2ConsTargetpos[i].res +
                                pos +
                                '" class="icn3d-residue" style="color:#' +
                                color +
                                '">' +
                        ...
                        if (emptyWidth >= 0) {
                            ...
                            html2 +=
                                '<div style="display:inline-block; background-color:#F00; width:' +
                                widthPerRes +
                                'px;" title="' +
                                c +
                                pos +
            ...
            htmlTmp =
                '<div class="icn3d-annoTitle" anno="sequence" chain="' +
                chnid +
                '"><span style="white-space:nowrap;" title="' +
                queryTitle +
                '">' +
                ...
                '</span></div>'
            htmlTmp +=
                '<span class="icn3d-residueNum" title="starting protein sequence number">' + ic.queryStart + '</span>'
            html3 += htmlTmp + '<br>'
            ...
            for (let i = 0, il = queryText.length; i < il; ++i) {
                ...
                if (c == ' ' || c == '-') {
                    ...
                    if (
                        ic.fullpos2ConsTargetpos !== undefined &&
                        ic.fullpos2ConsTargetpos[i] !== undefined &&
                        !ic.residues.hasOwnProperty(chnid + '_' + ic.fullpos2ConsTargetpos[i].pos)
                    ) {
                        c = c.toLowerCase()
    ...
    insertGap(chnid, seqIndex, text, bNohtml) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let html = ''
    ...
    insertGapOverview(chnid, seqIndex) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let html2 = ''
        if (
            me.cfg.blast_rep_id == chnid &&
            ic.targetGapHash !== undefined &&
            ic.targetGapHash.hasOwnProperty(seqIndex)
        ) {
            let width =
                (ic.seqAnnWidth * (ic.targetGapHash[seqIndex].to - ic.targetGapHash[seqIndex].from + 1)) /
                (ic.maxAnnoLength + ic.nTotalGap)
            html2 +=
                '<div style="display:inline-block; background-color:#333; width:' +
                width +
                'px; height:3px;">&nbsp;</div>'
        }
        return html2
    ...
    setAlternativeSeq(chnid, chnidBase) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    getProteinName(chnid) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let fullProteinName = ''
        if (
            (me.cfg.mmdbid !== undefined || me.cfg.gi !== undefined || me.cfg.blast_rep_id !== undefined) &&
            ic.mmdb_data !== undefined
        ) {
            ...
            (me.cfg.align !== undefined || me.cfg.chainalign !== undefined || ic.bRealign || ic.bSymd) &&
            ic.chainid2title !== undefined
        ) {
