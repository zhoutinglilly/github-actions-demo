...
class AddTrack {
    ...
    clickAddTrackButton() {
        ...
        let thisClass = this
        // ncbi gi/accession
        me.myEventCls.onIds('#' + ic.pre + 'addtrack_button1', 'click', function (e) {
        ...
        // FASTA
        me.myEventCls.onIds('#' + ic.pre + 'addtrack_button2', 'click', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            //e.preventDefault();
            dialog.dialog('close')
        ...
        // FASTA Alignment
        me.myEventCls.onIds('#' + ic.pre + 'addtrack_button2b', 'click', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            //e.preventDefault();
            dialog.dialog('close')
            ...
            for (let i = 0, il = ic.giSeq[chainid].length; i < il; ++i) {
                let pos =
                    i >= ic.matchedPos[chainid] && i - ic.matchedPos[chainid] < ic.chainsSeq[chainid].length
                        ? ic.chainsSeq[chainid][i - ic.matchedPos[chainid]].resi
                        : ic.baseResi[chainid] + 1 + i
            ...
            for (let i = 0, il = seqFirst.length; i < il; ++i) {
                if (seqFirst[i] == '-' && seqFirst[i] != prevSeq) {
                    // start of gap
                    from = cnt
                    dashCnt = 0
                }

                if (prevSeq == '-' && seqFirst[i] != prevSeq && cnt > 0) {
                    // end of gap
                    to = prevPos
                    ic.targetGapHash[from + startposGiSeq] = {
                        from: from + startposGiSeq,
                        to: to + dashCnt - 1 + startposGiSeq,
                    }
                }
            ...
            for (let j = 0, jl = trackSeqArray.length; j < jl; ++j) {
                ...
                let title =
                    trackTitleArray[j].length < 20 ? trackTitleArray[j] : trackTitleArray[j].substr(0, 20) + '...'
                let bMsa = true
            ...
            if (trackSeqArray.length > 0) {
                ...
                me.htmlCls.clickMenuCls.setLogCmd(
                    'color align custom | ' + chainid + ' | range ' + start + '_' + end + ' | ' + resiScoreStr,
                    true,
                )
            }
        })
        ...
        // BED file
        me.myEventCls.onIds('#' + ic.pre + 'addtrack_button3', 'click', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            //e.preventDefault();
            dialog.dialog('close')
        ...
        // custom
        me.myEventCls.onIds('#' + ic.pre + 'addtrack_button4', 'click', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            //e.preventDefault();
            dialog.dialog('close')
            ...
            me.htmlCls.clickMenuCls.setLogCmd(
                'add track | chainid ' + chainid + ' | title ' + title + ' | text ' + text + ' | type custom',
                true,
            )
        })

        // current selection
        me.myEventCls.onIds('#' + ic.pre + 'addtrack_button5', 'click', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            //e.preventDefault();
            dialog.dialog('close')
            ...
            for (let i = 0, il = ic.giSeq[chainid].length; i < il; ++i) {
                ...
                if (residueHash.hasOwnProperty(chainid + '_' + pos)) {
                    ...
                    let colorStr =
                        atom.color === undefined || atom.color.getHexString().toUpperCase() === 'FFFFFF'
                            ? 'DDDDDD'
    ...
    showNewTrack(chnid, title, text, cssColorArray, inTarget2queryHash, type, color, bMsa, fromArray, toArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        $('#' + ic.pre + 'dt_custom_' + chnid).append(
            "<div id='" + ic.pre + 'dt_custom_' + chnid + '_' + simpTitle + "'></div>",
        )
        $('#' + ic.pre + 'dt_custom_' + chnid + '_' + simpTitle).width(divLength)

        $('#' + ic.pre + 'ov_custom_' + chnid).append(
            "<div id='" + ic.pre + 'ov_custom_' + chnid + '_' + simpTitle + "'></div>",
        )
        $('#' + ic.pre + 'ov_custom_' + chnid + '_' + simpTitle).width(divLength)

        $('#' + ic.pre + 'tt_custom_' + chnid).append(
            "<div id='" + ic.pre + 'tt_custom_' + chnid + '_' + simpTitle + "'></div>",
        )
        ...
        let htmlTmp2 =
            '<div class="icn3d-seqTitle icn3d-link icn3d-blue" custom="' +
            (index + 1).toString() +
            '" from="' +
            fromArray +
            '" to="' +
        ...
        let bAlignColor =
            (type === undefined || type === 'seq' || type === 'custom') &&
            text.indexOf('cannot-be-aligned') == -1 &&
            text.indexOf('cannot be aligned') == -1
                ? true
                : false

        let bIdentityColor =
            type === 'identity' && text.indexOf('cannot-be-aligned') == -1 && text.indexOf('cannot be aligned') == -1
                ? true
                : false
            ...
            if (c != ' ' && c != '-') {
                ...
                let emptyWidth =
                    me.cfg.blast_rep_id == chnid
                        ? Math.round(
                              (ic.seqAnnWidth * i) / (ic.maxAnnoLength + ic.nTotalGap) - prevEmptyWidth - prevLineWidth,
                          )
                        : Math.round((ic.seqAnnWidth * i) / ic.maxAnnoLength - prevEmptyWidth - prevLineWidth)
        ...
        if (fromArray !== undefined) {
            ...
            let colorStr =
                atom.color === undefined || atom.color.getHexString() === 'FFFFFF'
                    ? 'DDDDDD'
                    : atom.color.getHexString()
            let color = atom.color !== undefined ? colorStr : 'CCCCCC'
            ...
            for (let i = 0, il = fromArray2.length; i < il; ++i) {
                ...
                let emptyWidth =
                    i == 0
                        ? Math.round(
                              (ic.seqAnnWidth * (fromArray2[i] - ic.baseResi[chnid] - 1)) /
                                  (ic.maxAnnoLength + ic.nTotalGap),
                          )
                        : Math.round(
                              (ic.seqAnnWidth * (fromArray2[i] - toArray2[i - 1] - 1)) /
                                  (ic.maxAnnoLength + ic.nTotalGap),
                          )
                htmlTmp2 += '<div style="display:inline-block; width:' + emptyWidth + 'px;">&nbsp;</div>'

                htmlTmp2 +=
                    '<div style="display:inline-block; color:white!important; font-weight:bold; background-color:#' +
                    color +
                    '; width:' +
                    Math.round(
                        (ic.seqAnnWidth * (toArray2[i] - fromArray2[i] + 1)) / (ic.maxAnnoLength + ic.nTotalGap),
                    ) +
                    'px;" class="icn3d-seqTitle icn3d-link icn3d-blue" custom="' +
                    (index + 1).toString() +
                    '" from="' +
                    fromArray2 +
                    '" to="' +
    ...
    alignSequenceToStructure(chainid, data, title) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let query, target
        ...
        if (query !== undefined && target !== undefined) {
            ...
            for (let i = 0, il = targetSeq.length; i < il; ++i) {
                if (target2queryHash.hasOwnProperty(i)) {
                    ...
                    let colorHexStr = ic.showAnnoCls.getColorhexFromBlosum62(
                        targetSeq[i],
                        querySeq[target2queryHash[i]],
                    )
    ...
    defineSecondary(chainid, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (
            !$('#' + ic.pre + 'dl_definedsets').hasClass('ui-dialog-content') ||
            !$('#' + ic.pre + 'dl_definedsets').dialog('isOpen')
        ) {
    ...
    simplifyText(text) {
            ...
            if (text[i] == '-' || text[i] == ' ') {
                if (bFoundText && i !== prevEmptyPos) {
                    if (prevEmptyPos + 1 == i - 1) {
                        out +=
                            (prevEmptyPos + 1 + 1).toString() +
                            ' ' +
                            text.substr(prevEmptyPos + 1, i - 1 - prevEmptyPos) +
                            ', '
                    } else {
                        out +=
                            (prevEmptyPos + 1 + 1).toString() +
        ...
        if (bFoundText && i == il) {
            if (prevEmptyPos + 1 == i - 1) {
                out +=
                    (prevEmptyPos + 1 + 1).toString() + ' ' + text.substr(prevEmptyPos + 1, i - 1 - prevEmptyPos) + ', '
            } else {
                out +=
                    (prevEmptyPos + 1 + 1).toString() +
                    '-' +
    ...
    checkGiSeq(chainid, title, text, type, color, bMsa, index) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        ...
        setTimeout(function () {
            thisClass.checkGiSeq(chainid, title, text, type, color, bMsa, index + 1)
        }, 100)
    }
    ...
    setCustomFile(type, startColor, midColor, endColor) {
        var ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        ...
        if (!file) {
            ...
            reader.onload = function (e) {
                ...
                if (type == 'color') {
                    ...
                    me.htmlCls.clickMenuCls.setLogCmd(
                        'color tube | ' + chainid + ' | range ' + start + '_' + end + ' | ' + resiScoreStr,
                        true,
                    )
                }
                ic.drawCls.draw()
