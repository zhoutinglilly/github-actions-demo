...
class LineGraph {
    ...
    drawLineGraph(lineGraphStr, bScatterplot) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let html,
            graph = JSON.parse(lineGraphStr)
        let linkArray = [],
            nodeArray1 = [],
            nodeArray2 = []
        let name2node = {}
        ...
        for (let i = 0, il = graph.links.length; i < il; ++i) {
            ...
            if (
                link.v == me.htmlCls.hbondValue ||
                link.v == me.htmlCls.ionicValue ||
                link.v == me.htmlCls.halogenValue ||
                link.v == me.htmlCls.picationValue ||
                link.v == me.htmlCls.pistackingValue ||
                link.v == me.htmlCls.contactValue
            ) {
                linkArray.push(link)
        ...
        if (Object.keys(ic.structures).length > 1) {
            ...
            html =
                strucArray.length == 0
                    ? 'No interactions found for each structure<br><br>'
            ...
            if (!bScatterplot) {
                ...
                html = linkArray.length > 0 ? '' : 'No interactions found for these two sets<br><br>'
                html +=
                    "<svg id='" + id + "' viewBox='0,0," + width + ',' + heightAll + "' width='" + graphWidth + "px'>"
    ...
    drawLineGraph_base(nodeArray1, nodeArray2, linkArray, name2node, height) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let html = ''
        let len1 = nodeArray1.length,
    ...
    drawScatterplot_base(nodeArray1, nodeArray2, linkArray, name2node, height, bContactMap) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let html = ''
        let len1 = nodeArray1.length,
    ...
    drawOnePairNode(link, node1, node2, node2posSet1, node2posSet2, bContactMap) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let html = ''
        ...
        if (bContactMap) {
            html +=
                "<rect x='" +
                (pos2.x - halfSize).toString() +
                "' y='" +
                (pos1.y - halfSize).toString() +
                "' width='" +
                rectSize +
                "' height='" +
                rectSize +
