...
class ShowInter {
    ...
    showInteractions(type) {
        ...
        if (nameArray2.length == 0) {
            ...
            let thresholdStr =
                ...
                ' ' +
                thresholdPistacking
            let result = ic.viewInterPairsCls.viewInteractionPairs(
                nameArray2,
                nameArray,
                ic.bHbondCalc,
                type,
            ...
            let tmpStr =
                nameArray2 + ' ' + nameArray + ' | ' + interactionTypes + ' | ' + bHbondCalcStr + ' | ' + thresholdStr
            if (type == '3d') {
    ...
    showHbonds(threshold, nameArray2, nameArray, bHbondCalc, bSaltbridge, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    showIonicInteractions(threshold, nameArray2, nameArray, bHbondCalc, bSaltbridge, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (bHbondCalc) return
        let hbonds_saltbridge, select
    ...
    showHalogenPi(threshold, nameArray2, nameArray, bHbondCalc, type, interactionType) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (bHbondCalc) return
    ...
    showSsbonds() {
        ...
        for (let s = 0, sl = structureArray.length; s < sl; ++s) {
            ...
            for (let i = 0, lim = Math.floor(ic.ssbondpnts[structure].length / 2); i < lim; i++) {
                let res1 = ic.ssbondpnts[structure][2 * i],
                    res2 = ic.ssbondpnts[structure][2 * i + 1]
                residues[res1] = 1
                residues[res2] = 1
    ...
    pickCustomSphere(radius, nameArray2, nameArray, bSphereCalc, bInteraction, type) {
        let ic = this.icn3d,
        ...
        let bGetPairs = true
        let result = this.pickCustomSphere_base(
            radius,
            atomlistTarget,
            otherAtoms,
            bSphereCalc,
    ...
    pickCustomSphere_base(
        ...
        let atoms
        if (bInteraction) {
            atoms = ic.contactCls.getAtomsWithinAtom(
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, otherAtoms, ic.atoms),
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, atomlistTarget, ic.atoms),
                parseFloat(radius),
                bGetPairs,
                bInteraction,
                undefined,
