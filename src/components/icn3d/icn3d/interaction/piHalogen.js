...
class PiHalogen {
    ...
    calculateHalogenPiInteractions(startAtoms, targetAtoms, threshold, type, interactionType, bInternal) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        for (let i in atoms1a) {
            let atom1 = atoms1a[i]
            let oriResidName =
                atom1.resn + ' $' + atom1.structure + '.' + atom1.chain + ':' + atom1.resi + '@' + atom1.name
            ...
            for (let j in atoms1b) {
                ...
                let bResult = this.getHalogenPiInteractions(
                    atom1,
                    atom2,
                    type,
                    interactionType,
                ...
                if (bResult) {
                    hbondsAtoms = me.hashUtilsCls.unionHash(
                        hbondsAtoms,
                        ic.residues[atom1.structure + '_' + atom1.chain + '_' + atom1.resi],
                    )
                    hbondsAtoms = me.hashUtilsCls.unionHash(
                        hbondsAtoms,
                        ic.residues[atom2.structure + '_' + atom2.chain + '_' + atom2.resi],
                    )
    ...
    getHalogenPiInteractions(atom1, atom2, type, interactionType, threshold, maxlengthSq, oriResidName, bInternal) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    getAromaticRings(resn, resid, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let piPosArray = []
        let normalArray = []
        ...
        if (type == 'nucleotide') {
            ...
            if (
                resn.trim().toUpperCase() == 'A' ||
                resn.trim().toUpperCase() == 'DA' ||
                resn.trim().toUpperCase() == 'G' ||
                resn.trim().toUpperCase() == 'DG'
            ) {
                ...
                resn.trim().toUpperCase() == 'C' ||
                resn.trim().toUpperCase() == 'DC' ||
                resn.trim().toUpperCase() == 'T' ||
                resn.trim().toUpperCase() == 'DT' ||
                resn.trim().toUpperCase() == 'U' ||
                resn.trim().toUpperCase() == 'DU'
            ) {
                for (let i in ic.residues[resid]) {
                    let atom = ic.atoms[i]
                    if (atom.name == 'N1' || atom.name == 'C2' || atom.name == 'N3' || atom.name == 'C6') {
    ...
    getAromaticPisLigand(resid) {
        ...
        for (let i = 1; i <= cyclenumber; i++) {
            ...
            if (cnt == 5 || cnt == 6) {
                ...
                if (Math.abs(bPlane) < 0.052) {
                    // same plane, 87-90 degree
                    coord.multiplyScalar(1.0 / cnt)

                    let atom = ic.atoms[serial]
                    name2atom[resid + '_pi' + serial] = {
                        resn: atom.resn,
                        name: 'pi' + serial,
                        coord: coord,
                        serial: atom.serial,
