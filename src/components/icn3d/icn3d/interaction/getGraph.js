...
class GetGraph {
    ...
    getGraphData(atomSet2, atomSet1, nameArray2, nameArray, html, labelType, bAnyAtom) {
        let ic = this.icn3d,
            me = ic.icn3dui
            ...
            for (let i = 0, il = ic.ssbondpnts[structure].length; i < il; i += 2) {
                ...
                if (selectedAtoms.hasOwnProperty(atom1.serial) && selectedAtoms.hasOwnProperty(atom2.serial)) {
                    ...
                    disulfideLinkStr +=
                        ...
                        resName2 +
                        '", "v": ' +
                        me.htmlCls.ssbondValue +
                        ', "c": "' +
                        me.htmlCls.ssbondColor +
                        '"}'
                }
            }
        }
            ...
            for (let i = 0, il = ic.clbondpnts[structure].length; i < il; i += 2) {
                ...
                if (selectedAtoms.hasOwnProperty(atom1.serial) && selectedAtoms.hasOwnProperty(atom2.serial)) {
                    ...
                    crossLinkStr +=
                        ...
                        resName2 +
                        '", "v": ' +
                        me.htmlCls.clbondValue +
                        ', "c": "' +
                        me.htmlCls.clbondColor +
                        '"}'
                }
            }
        }
    ...
    drawResNode(node, i, r, gap, margin, y, setName, bVertical, bContactMap) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let x,
            resid = node.r.substr(4)
        if (bVertical) {
            x = margin - i * (r + gap)
        } else {
            x = margin + i * (r + gap)
    ...
    getNodeTopBottom(nameHash, name2node, bReverseNode) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        let nodeArray1 = [],
    ...
    updateGraphJson(struc, index, nodeArray1, nodeArray2, linkArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let lineGraphStr = ''
    ...
    getNodesLinksForSet(atomSet, labelType, setName, bAnyAtom) {
        let ic = this.icn3d,
            me = ic.icn3dui
        //var nodeStr = '', linkStr = '';
        let nodeArray = [],
            ...
            if (
                ...
                nodeArray.push(
                    '{"id": "' +
                        ...
                        setName +
                        '", "x": ' +
                        atom.coord.x.toFixed(0) +
                        ', "y": ' +
                        atom.coord.y.toFixed(0) +
                        ', "c": "' +
                        atom.color.getHexString().toUpperCase() +
                        '"}',
                )
                if (
                    cnt > 0 &&
                    prevChain == atom.chain &&
                    (parseInt(atom.resi) == parseInt(prevResi) + 1 || atom.resi == prevResi)
                ) {
    ...
    getHbondLinksForSet(atoms, labelType) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        if (Object.keys(complement).length > 0 && Object.keys(firstSetAtoms).length > 0) {
            ...
            let selectedAtoms = ic.hBondCls.calculateChemicalHbonds(
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, complement, ic.atoms),
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, firstSetAtoms, ic.atoms),
        ...
        let hbondStr = this.getGraphLinks(
            resid2ResidhashHbond,
            resid2ResidhashHbond,
            me.htmlCls.hbondInsideColor,
            labelType,
            me.htmlCls.hbondValuehbondInsideValue,
        )
        return hbondStr
    }
    getIonicLinksForSet(atoms, labelType) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let resid2Residhash = {}
        ...
        if (Object.keys(complement).length > 0 && Object.keys(firstSetAtoms).length > 0) {
            let bSaltbridge = false
            let selectedAtoms = ic.saltbridgeCls.calculateIonicInteractions(
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, complement, ic.atoms),
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, firstSetAtoms, ic.atoms),
                parseFloat(threshold),
                bSaltbridge,
        ...
        let ionicStr = this.getGraphLinks(
            resid2Residhash,
            resid2Residhash,
            me.htmlCls.ionicInsideColor,
            labelType,
            me.htmlCls.ionicInsideValue,
        )
        return ionicStr
    }
    getHalogenPiLinksForSet(atoms, labelType) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let resid2Residhash = {}
        let firstSetAtoms = atoms
        ...
        if (Object.keys(complement).length > 0 && Object.keys(firstSetAtoms).length > 0) {
            let selectedAtoms = ic.piHalogenCls.calculateHalogenPiInteractions(
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, firstSetAtoms, ic.atoms),
                me.hashUtilsCls.intHash2Atoms(ic.dAtoms, complement, ic.atoms),
                parseFloat(threshold),
                'graph',
        ...
        halogenpiStr += this.getGraphLinks(
            resid2Residhash,
            resid2Residhash,
            me.htmlCls.halogenInsideColor,
            labelType,
            me.htmlCls.halogenInsideValue,
        )
        ...
        halogenpiStr += this.getGraphLinks(
            resid2Residhash,
            resid2Residhash,
            me.htmlCls.picationInsideColor,
            labelType,
            me.htmlCls.picationInsideValue,
        )
        ...
        halogenpiStr += this.getGraphLinks(
            resid2Residhash,
            resid2Residhash,
            me.htmlCls.pistackingInsideColor,
            labelType,
            me.htmlCls.pistackingInsideValue,
        )
        return halogenpiStr
    }
    getContactLinksForSet(atoms, labelType, bCartoon2d) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let ssAtomsArray = []
        let prevSS = '',
    ...
    getContactLinks(atomlistTarget, otherAtoms, labelType, bInternal, bCartoon2d) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let radius = parseFloat($('#' + ic.pre + 'contactthreshold').val())
        let bGetPairs = true,
            bInteraction = false
        let atoms = ic.contactCls.getAtomsWithinAtom(
            otherAtoms,
            atomlistTarget,
            parseFloat(radius),
            bGetPairs,
        ...
        let interStr = this.getGraphLinks(
            residHash,
            residHash,
            me.htmlCls.contactInsideColor,
            labelType,
            me.htmlCls.contactInsideValue,
            bCartoon2d,
        )
        return interStr
    }
    compNode(a, b, bReverseChain) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    getGraphLinks(hash1, hash2, color, labelType, value, bCartoon2d) {
        var ic = this.icn3d,
            me = ic.icn3dui
        let hbondStr = ''
        ...
        for (let resid1 in hash1) {
            ...
            let resName1 =
                me.utilsCls.residueName2Abbr(resid1.substr(0, pos1a)) + resid1.substr(pos1b + 1, pos1c - pos1b - 1)
            if (labelType == 'chain' || labelType == 'structure')
                resName1 += '.' + resid1.substr(pos1d + 1, pos1b - pos1d - 1)
            ...
            for (let resid2 in hash2[resid1]) {
                ...
                let resName2 =
                    me.utilsCls.residueName2Abbr(resid2.substr(0, pos2a)) + resid2.substr(pos2b + 1, pos2c - pos2b - 1) //
                ;+'_' + resid2.substr(pos2d + 1, pos2b - pos2d - 1)
                if (labelType == 'chain' || labelType == 'structure')
                    resName2 += '.' + resid2.substr(pos2d + 1, pos2b - pos2d - 1)
                ...
                if (
                    !sourceTargetHash.hasOwnProperty(resName1 + '_' + resName2) &&
                    resName1 !== undefined &&
                    resName2 !== undefined
                ) {
                    let linkStr =
    ...
    convertLabel2Resid(residLabel) {
        ...
        let resid =
            residLabel.substr(pos3 + 1, pos4 - pos3 - 1) +
            '_' +
            residLabel.substr(pos4 + 1, pos5 - pos4 - 1) +
            '_' +
            residLabel.substr(pos5 + 1, pos2 - pos5 - 1)
        return resid
    }
}
