    ...
    viewInteractionPairs(
        ...
        if (bContactMapLocal) {
            ...
            for (let i in ic.hAtoms) {
                ...
                if (
                    (type == 'calpha' && (atom.het || atom.name == 'CA' || atom.name == "O3'" || atom.name == 'O3*')) ||
        ...
        if (bInteraction) {
            ...
            if (!(nameArray2.length == 1 && nameArray.length == 1 && nameArray2[0] == nameArray[0])) {
                ...
                // contact in a set, atomSet1 same as atomSet2
                if (!bHbondCalc) {
                    let residues = {}
                    ...
                    if (bContactMapLocal) {
                        ...
                        let result = ic.showInterCls.pickCustomSphere_base(
                            threshold,
                            atomSet1,
                            atomSet2,
                            bHbondCalc,
                        ...
                        for (let resid in result.resid2Residhash) {
                            resid2ResidhashInteractions[resid] = me.hashUtilsCls.unionHash(
                                resid2ResidhashInteractions[resid],
                                result.resid2Residhash[resid],
                            )
                        }
                    } else {
                        let ssAtomsArray = []
                    ...
                    if (firstAtom !== undefined) {
                        commandname =
                            ...
                            firstAtom.chain +
                            ':' +
                            me.utilsCls.residueName2Abbr(firstAtom.resn.substr(0, 3)).trim() +
                            firstAtom.resi +
                            '-' +
                            radius +
                            'A'
        ...
        html +=
            ...
            'selset" cmd="' +
            cmd2 +
            '">Highlight in 3D</button><br><br></div>'
        ...
        html +=
            'interface_2 <button class="' + ic.pre + 'selset" cmd="' + cmd4 + '">Highlight in 3D</button><br><br></div>'
        html +=
            '<div><b>Note</b>: Each checkbox below selects the corresponding residue. ' +
        ...
        if (type == 'save1' || type == 'save2') {
            ...
            html +=
                '<div style="text-align:center"><br><b>Interactions Sorted on ' +
                tmpText +
                '</b>: <button class="' +
                ic.pre +
                'showintercntonly" style="margin-left:20px">Show Count Only</button><button class="' +
                ic.pre +
                'showinterdetails" style="margin-left:20px">Show Details</button></div>'
            ...
            let bLine = true
            let bAnyAtom = true
            let graphStr = ic.getGraphCls.getGraphData(
                atomSet1,
                atomSet2,
                nameArray2,
                nameArray,
...
*/
    ...
    retrieveInteractionData() {
        ...
        if (!ic.b2DShown) {
            if (me.cfg.align !== undefined) {
                ...
                ic.ParserUtilsCls.set2DDiagramsForAlign(
                    structureArray[0].toUpperCase(),
                    structureArray[1].toUpperCase(),
                )
    ...
    getAllInteractionTable(type) {
        ...
        for (let i = 0, il = residsArray.length; i < il; ++i) {
            ...
            if (i > 0 && residname1 != prevResidname1) {
                bondCnt.push({
                    ...
                    cntHalegen: cntHalegen,
                    cntPication: cntPication,
                    cntPistacking: cntPistacking,
                })

                tmpText += this.getInteractionPerResidue(
                    prevIds,
                    strHbond,
                    strIonic,
                    strContact,
        ...
        if (residsArray.length > 0) {
            ...
            html +=
                '<tr><th rowspan=2>Residue</th><th rowspan=2># Hydrogen<br>Bond</th><th rowspan=2># Salt Bridge<br>/Ionic Interaction</th><th rowspan=2># Contact</th>'
            html +=
                '<th rowspan=2># Halogen<br>Bond</th><th rowspan=2># &pi;-Cation</th><th rowspan=2># &pi;-Stacking</th>'
            ...
            html += '<tr>'
            let tmpStr =
                '<td><table width="100%" class="icn3d-border"><tr><td>Atom1</td><td>Atom2</td><td>Distance(&#8491;)</td><td>Highlight in 3D</td></tr></table></td>'
            html += tmpStr
            html += tmpStr
            html +=
                '<td><table width="100%" class="icn3d-border"><tr><td>Atom1</td><td>Atom2</td><td># Contacts</td><td>Min Distance(&#8491;)</td><td>C-alpha Distance(&#8491;)</td><td>Highlight in 3D</td></tr></table></td>'
            html += tmpStr
            html += tmpStr
            html += tmpStr
            html += '</tr>'
    ...
    getInteractionPairDetails(labels2dist, type, interactionType) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let tmpText = '',
            cnt = 0
        ...
        if (labels2dist !== undefined) {
            for (let labels in labels2dist) {
                ...
                tmpText +=
                    '<tr><td><span style="white-space:nowrap"><input type="checkbox" class="' +
                    ic.pre +
                    'seloneres" id="' +
                    ic.pre +
                    interactionType +
                    ...
                    color1 +
                    colorText2 +
                    '</span></td><td><span style="white-space:nowrap"><input type="checkbox" class="' +
                    ic.pre +
                    'seloneres" id="' +
                    ic.pre +
                    interactionType +
    ...
    getContactPairDetails(labels2dist, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let tmpText = '',
            cnt = 0
    ...
    exportInteractions() {
        var ic = this.icn3d,
            me = ic.icn3dui
        let text =
            '<html><body><div style="text-align:center"><br><b>Interacting residues</b>:<br/><table align=center border=1 cellpadding=10 cellspacing=0><tr><th>Base Chain: Residues</th><th>Interacting Chain</th></tr>'
        for (let fisrtChainid in ic.chainname2residues) {
            for (let name in ic.chainname2residues[fisrtChainid]) {
                let secondChainid =
                    fisrtChainid.substr(0, fisrtChainid.indexOf('_')) + '_' + name.substr(0, name.indexOf(' '))
    ...
    exportSsbondPairs() {
        ...
        let text =
            '<html><body><div style="text-align:center"><br><b>' +
            cnt +
            ' disulfide pairs</b>:<br><br><table align=center border=1 cellpadding=10 cellspacing=0><tr><th>Residue ID 1</th><th>Residue ID 2</th></tr>'
        text += tmpText
        text += '</table><br/></div></body></html>'
        let file_pref = ic.inputid ? ic.inputid : 'custom'
    ...
    exportClbondPairs() {
        ...
        for (let structure in ic.structures) {
            ...
            for (let i = 0, il = clbondArray.length; i < il; i = i + 2) {
                ...
                if (!residHash.hasOwnProperty(resid1 + '_' + resid2)) {
                    ...
                    tmpText +=
                        '<tr><td>' + resid1 + ' ' + atom1.resn + '</td><td>' + resid2 + ' ' + atom2.resn + '</td></tr>'
                    ++cnt
                }
        ...
        let text =
            '<html><body><div style="text-align:center"><br><b>' +
            cnt +
            ' cross-linkage pairs</b>:<br><br><table align=center border=1 cellpadding=10 cellspacing=0><tr><th>Residue ID 1</th><th>Residue ID 2</th></tr>'
        text += tmpText
        text += '</table><br/></div></body></html>'
        let file_pref = ic.inputid ? ic.inputid : 'custom'
        ic.saveFileCls.saveFile(file_pref + '_crosslinkage_pairs.html', 'html', text)
    }
    exportHbondPairs(type, labelType) {
        var ic = this.icn3d,
            me = ic.icn3dui
        let tmpText = ''
        let cnt = 0
        ...
        for (let resid1 in ic.resid2ResidhashHbond) {
            ...
            for (let resid2 in ic.resid2ResidhashHbond[resid1]) {
                ...
                tmpText +=
                    '<tr><td><input type="checkbox" class="' +
                    ic.pre +
                    'seloneres" id="' +
                    ic.pre +
                    'hbond_' +
                    ...
                    color1 +
                    colorText2 +
                    '</td><td><input type="checkbox" class="' +
                    ic.pre +
                    'seloneres" id="' +
                    ic.pre +
                    'hbond_' +
        ...
        let text = '<div style="text-align:center"><br><b>' + cnt + ' hydrogen bond pairs</b>:</div><br>'
        if (cnt > 0) {
            text +=
                '<br><table align=center border=1 cellpadding=10 cellspacing=0>' +
        ...
        if (type == 'graph' || type == 'linegraph' || type == 'scatterplot') {
            let hbondStr = ic.getGraphCls.getGraphLinks(
                ic.resid2ResidhashHbond,
                ic.resid2ResidhashHbond,
                me.htmlCls.hbondColor,
                labelType,
                me.htmlCls.hbondValue,
            )
            return hbondStr
        } else {
            return text
    ...
    exportSaltbridgePairs(type, labelType) {
        var ic = this.icn3d,
            me = ic.icn3dui
        let tmpText = ''
        let cnt = 0
        ...
        let text =
            '<div style="text-align:center"><br><b>' + cnt + ' salt bridge/ionic interaction pairs</b>:</div><br>'
        if (cnt > 0) {
            text +=
                '<br><table align=center border=1 cellpadding=10 cellspacing=0>' +
                '<tr><th>Atom 1</th><th>Atom 2</th><th>Distance(&#8491;)</th>'
        ...
        if (type == 'graph' || type == 'linegraph' || type == 'scatterplot') {
            let hbondStr = ic.getGraphCls.getGraphLinks(
                ic.resid2ResidhashSaltbridge,
                ic.resid2ResidhashSaltbridge,
                me.htmlCls.ionicColor,
                labelType,
                me.htmlCls.ionicValue,
            )
            return hbondStr
        } else {
            return text
    ...
    exportHalogenPiPairs(type, labelType, interactionType) {
        var ic = this.icn3d,
            me = ic.icn3dui
        let tmpText = ''
        let cnt = 0
        ...
        let text = '<div style="text-align:center"><br><b>' + cnt + ' ' + interactionType + ' pairs</b>:</div><br>'
        if (cnt > 0) {
            text +=
                '<br><table align=center border=1 cellpadding=10 cellspacing=0>' +
                '<tr><th>Atom 1</th><th>Atom 2</th><th>Distance(&#8491;)</th>'
    ...
    exportSpherePairs(bInteraction, type, labelType) {
        var ic = this.icn3d,
            me = ic.icn3dui
        let tmpText = ''
        let cnt = 0
        ...
        let colorText1 = ' <span style="background-color:#'
        let colorText2 = '">&nbsp;&nbsp;&nbsp;</span>'
        for (let resid1 in residHash) {
            // e.g., resid1: TYR $1KQ2.A:42
            let resid1Real = ic.getGraphCls.convertLabel2Resid(resid1)
            let atom1 = ic.firstAtomObjCls.getFirstAtomObj(ic.residues[resid1Real])
            let color1 = atom1.color ? atom1.color.getHexString() : ''
        ...
        let text = '<div style="text-align:center"><br><b>' + cnt + ' residue pairs in ' + nameStr + '</b>:</div><br>'
        if (cnt > 0) {
            if (bInteraction) {
                text +=
                    '<br><table align=center border=1 cellpadding=10 cellspacing=0>' +
        ...
        if (
            ...
            let interStr = ic.getGraphCls.getGraphLinks(
                residHash,
                residHash,
                me.htmlCls.contactColor,
                labelType,
                me.htmlCls.contactValue,
            )
            return interStr
        } else {
            return text
