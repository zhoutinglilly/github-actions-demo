...
class HBond {
    ...
    isHbondDonorAcceptor(atom) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (
            (atom.name == 'N' && !atom.het) || // backbone
        ...
        // if the neighboring C of Oxygen has two or more bonds with O or N, the oxygen is an acceptor
            ...
            for (let k = 0, kl = cAtom.bonds.length; k < kl; ++k) {
                if (
                    ic.atoms[cAtom.bonds[k]].elem == 'O' ||
                    ic.atoms[cAtom.bonds[k]].elem == 'N' ||
                    ic.atoms[cAtom.bonds[k]].elem == 'S'
                ) {
                    ++cnt
                }
            }
    ...
    isValidHbond(atom, atomHbond, threshold) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // return: 'donor', 'acceptor', 'both', 'ring', 'none'
    ...
    calculateChemicalHbonds(startAtoms, targetAtoms, threshold, bSaltbridge, type, bInternal) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        for (let i in startAtoms) {
            ...
            let bAtomCond = bSaltbridge
                ? (atom.resn === 'LYS' && atom.elem === 'N' && atom.name !== 'N') ||
                ...
                : atom.elem === 'N' ||
                  atom.elem === 'O' ||
                  (atom.elem === 'S' && (atom.het || atom.resn === 'Cys' || atom.resn === 'Met'))
        ...
        for (let i in targetAtoms) {
            ...
            if (bAtomCond) {
                ...
                let oriResidName =
                    atom.resn + ' $' + atom.structure + '.' + atom.chain + ':' + atom.resi + '@' + atom.name
                ...
                for (let j in atomHbond) {
                    if (bSaltbridge) {
                        ...
                        if (
                            ((atom.resn === 'LYS' || atom.resn === 'ARG') &&
                                (atomHbond[j].resn === 'LYS' || atomHbond[j].resn === 'ARG')) ||
                            ((atom.resn === 'GLU' || atom.resn === 'ASP') &&
                                (atomHbond[j].resn === 'GLU' || atomHbond[j].resn === 'ASP'))
                        ) {
                            continue
                        }
                    }
                    ...
                    if (
                        ...
                        ic.proteins.hasOwnProperty(atomHbond[j].serial) &&
                        (atom.name === 'N' || atom.name === 'O') &&
                        (atomHbond[j].name === 'O' || atomHbond[j].name === 'N')
                    ) {
                        if (atom.name === atomHbond[j].name) continue
                        if (
                            atom.structure == atomHbond[j].structure &&
                            atom.chain == atomHbond[j].chain &&
                            Math.abs(atom.resi - atomHbond[j].resi) <= 1
                        )
                            continue // peptide bond
    ...
    setHbondsContacts(options, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        if (options[hbonds_contact].toLowerCase() === 'yes') {
            ...
            for (let i = 0, lim = Math.floor(pnts.length / 2); i < lim; i++) {
                ...
                if (
                    line.serial1 !== undefined &&
                    line.serial2 !== undefined &&
                    !ic.dAtoms.hasOwnProperty(line.serial1) &&
                    !ic.dAtoms.hasOwnProperty(line.serial2)
                )
                    continue
