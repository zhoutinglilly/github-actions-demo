...
ElectronMap.prototype.getFacesAndVertices = function (allatoms, atomlist) {
    let atomsToShow = {}
    let i, il
    for (i = 0, il = atomlist.length; i < il; i++) atomsToShow[atomlist[i]] = 1
    let vertices = this.verts

    for (i = 0, il = vertices.length; i < il; i++) {
        let r
        if (this.type == 'phi') {
            r = new THREE.Vector3(vertices[i].x, vertices[i].y, vertices[i].z)
                .multiplyScalar(1.0 / this.header.scale)
...
ElectronMap.prototype.initparm = function (
    inHeader,
    inData,
    inMatrix,
    inIsovalue,
    ...
    if (this.header && this.header.max !== undefined) {
        // EM density map from EBI
        this.isovalue = this.header.min + ((this.header.max - this.header.min) * inIsovalue) / 100.0
    } else if (this.header && this.header.mean !== undefined) {
        // density map from EBI
    ...
    this.pminx = 0
    this.pmaxx = this.header.xExtent - 1
    this.pminy = 0
    this.pmaxy = this.header.yExtent - 1
    this.pminz = 0
    this.pmaxz = this.header.zExtent - 1
...
ElectronMap.prototype.fillvoxels = function (atoms, atomlist) {
    //(int seqinit,int
    // seqterm,bool
    // atomthis.type,atom*
    // proseq,bool bcolor)
    ...
    if (this.type == 'phi' && !this.header.bSurface) {
        // equipotential map
            ...
            for (j = 0; j < this.pWidth; ++j) {
                for (k = 0; k < this.pHeight; ++k) {
                    ...
                    if (index2 < this.dataArray.length) {
                        this.vpBits[index] =
                            this.dataArray[index2] >= this.isovalue || this.dataArray[index2] <= -this.isovalue ? 1 : 0
        ...
        if (this.type == 'phi' && this.header.bSurface) {
            ...
            for (let serial in atomlist) {
                ...
                if (this.loadPhiFrom != 'delphi') {
                    // transform to the original position if the potential file is imported
                    if (this.rmsd_supr !== undefined && this.rmsd_supr.rot !== undefined) {
            ...
            for (i = 0, il = indexArray.length; i < il; ++i) {
                ...
                if (this.type == '2fofc') {
                    ...
                    this.vpBits[index] =
                        this.dataArray[index] >= this.isovalue || this.dataArray[index] <= -this.isovalue ? 1 : 0
                    this.vpAtomID[index] = this.dataArray[index] >= 0 ? 1 : 0 // determine whether it's positive
