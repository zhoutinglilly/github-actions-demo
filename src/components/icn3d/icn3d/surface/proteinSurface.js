...
ProteinSurface.prototype.inOrigExtent = function (x, y, z) {
    if (x < this.origextent[0][0] || x > this.origextent[1][0]) return false
    if (y < this.origextent[0][1] || y > this.origextent[1][1]) return false
    if (z < this.origextent[0][2] || z > this.origextent[1][2]) return false
    return true
}
...
ProteinSurface.prototype.initparm = function (
    extent,
    btype,
    in_bCalcArea,
    atomlist,
    ...
    for (let i = 0, il = atomlist.length; i < il; i++) this.atomsToShow[atomlist[i]] = 1
...
ProteinSurface.prototype.boundingatom = function (btype) {
    ...
    for (let i in this.vdwRadii) {
        if (!this.vdwRadii.hasOwnProperty(i)) continue
        let r = this.vdwRadii[i]
        if (!btype) tradius[i] = r * this.scaleFactor + 0.5
        else tradius[i] = (r + this.probeRadius) * this.scaleFactor + 0.5
        ...
        for (let j = 0; j < this.widxz[i]; j++) {
            for (let k = 0; k < this.widxz[i]; k++) {
                txz = j * j + k * k
                if (txz > sradius) this.depty[i][indx] = -1
                // outside
                else {
...
ProteinSurface.prototype.fillvoxels = function (atoms, atomlist) {
    //(int seqinit,int
    // seqterm,bool
    // atomtype,atom*
    // proseq,bool bcolor)
    ...
    for (i in atomlist) {
        ...
        if (atom === undefined || atom.resn === 'DUM') continue
    ...
    if (this.dataArray) {
        let pminx2 = 0,
            pmaxx2 = this.header.xExtent - 1
        let pminy2 = 0,
            pmaxy2 = this.header.yExtent - 1
        let pminz2 = 0,
            pmaxz2 = this.header.zExtent - 1
        ...
        for (i = 0; i < pLength2; ++i) {
            for (j = 0; j < pWidth2; ++j) {
                for (k = 0; k < pHeight2; ++k) {
                    ...
                    let index2
                    if (this.header.filetype == 'phi') {
                        // loop z, y, x
                        index2 = k * widthHeight2 + j * height2 + i
                    } else if (this.header.filetype == 'cube') {
                        // loop x, y, z
        ...
        for (i = 0; i < this.pLength; ++i) {
            for (j = 0; j < this.pWidth; ++j) {
                for (k = 0; k < this.pHeight; ++k) {
                    ...
                    let nx0 = Math.floor(r.x),
                        nx1 = Math.ceil(r.x)
                    let ny0 = Math.floor(r.y),
                        ny1 = Math.ceil(r.y)
                    let nz0 = Math.floor(r.z),
                        nz1 = Math.ceil(r.z)
    ...
    for (i = 0, il = this.vpBits.length; i < il; i++) if (this.vpBits[i] & this.INOUT) this.vpBits[i] |= this.ISDONE
}
...
ProteinSurface.prototype.fillvoxelswaals = function (atoms, atomlist) {
    let i, il
    for (i = 0, il = this.vpBits.length; i < il; i++) this.vpBits[i] &= ~this.ISDONE // not isdone
...
ProteinSurface.prototype.fastoneshell = function (inarray, boundPoint) {
    //(int* innum,int
    // *allocout,voxel2
    // ***boundPoint, int*
    // outnum, int *elimi)
    ...
    this.print = function () {
        ...
        for (let i = 0; i < table.length; i++) {
            ...
            for (let j = 0; j < table[i].length; j += 3) {
                ...
                if (typeof data[i][k] === 'undefined') console.log('undef ' + i + ',' + k)
            }
...
ProteinSurface.prototype.marchingcube = function (stype) {
    ...
    for (let i = 0, vlen = this.verts.length; i < vlen; i++) {
        this.verts[i]['atomid'] = this.vpAtomID[
            this.verts[i].x * pWH + this.pHeight * this.verts[i].y + this.verts[i].z
        ]
        if (this.dataArray)
            this.verts[i]['color'] = this.vpColor[
                this.verts[i].x * pWH + this.pHeight * this.verts[i].y + this.verts[i].z
            ]
        if (this.dataArray)
            this.verts[i]['pot'] = this.vpPot[this.verts[i].x * pWH + this.pHeight * this.verts[i].y + this.verts[i].z]
    }
    ...
    if (this.bCalcArea) {
        ...
        for (let i = 0, il = this.faces.length; i < il; i += 3) {
            ...
            let ai = this.verts[fa]['atomid'],
                bi = this.verts[fb]['atomid'],
                ci = this.verts[fc]['atomid']
            ...
            let currArea = 0
            if (parseInt((max - min) * 100) == 0) {
                // case 2
                currArea = 0.433 * min
            } else if (parseInt((mid - min) * 100) == 0) {
                // case 1
                currArea = 0.5 * min
            } else {
                // case 3
