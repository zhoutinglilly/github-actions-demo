...
class ChainalignParser {
    ...
    downloadChainalignmentPart2(data1, data2Array, chainresiCalphaHash2, chainidArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    downloadChainalignmentPart3(chainresiCalphaHash2, chainidArray, hAtoms) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // select all
        let allAtoms = {}
        ...
        if (chainresiCalphaHash2 !== undefined)
            ic.ParserUtilsCls.transformToOpmOriForAlign(ic.selectedPdbid, chainresiCalphaHash2, true)
    ...
    downloadChainalignment(chainalign, resnum, resdef) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        ...
        $.when
            ...
            .fail(function () {
                alert(
                    'These chains can not be aligned by VAST server. You can specify the residue range and try it again...',
                )
                //          thisClass.parseChainAlignData(arguments, alignArray, ic.mmdbid_t, ic.chain_t);
            })
    }

    parseChainAlignData(dataArray, chainidArray, mmdbid_t, chain_t) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        ...
        for (let index = 1, indexl = dataArray.length; index < indexl; index += step) {
            ...
            if (me.cfg.resnum || me.cfg.resdef) {
                ...
                if (
                    queryData !== undefined &&
                    JSON.stringify(queryData).indexOf('Oops there was a problem') === -1 &&
                    align !== undefined &&
                    JSON.stringify(align).indexOf('Oops there was a problem') === -1
                ) {
                    if ((align === undefined || align.length == 0) && mmdbid_q == mmdbid_t && chain_q == chain_t) {
                        ...
                        if (!me.cfg.command)
                            alert(
                                'These two chains ' +
                                    chainidArray[index2] +
                                    ' can not align to each other. ' +
                                    'Please select sequences from these two chains in the "Sequences & Annotations" window, ' +
                                    'and click "Realign Selection" in the "File" menu to align your selection.',
                            )
    ...
    loadOpmDataForChainalign(data1, data2, chainidArray, mmdbidArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
