...
class MmcifParser {
    ...
    downloadMmcifSymmetry(mmcifid, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    downloadMmcifSymmetryBase(mmcifid, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        ...
        if (me.utilsCls.isMac()) {
            // safari has a problem in getting data from https://files.rcsb.org/header/
            url = 'https://files.rcsb.org/view/' + mmcifid + '.cif'
        } else {
                ...
                if (me.cfg.notebook) {
                    ...
                    for (let i in lines) {
                        ...
                        if (line.substr(0, 10) == 'EMDB  EMD-') {
                            ...
                            bStartOrganism &&
                            !bOrganism &&
                            prevLine.substr(0, 23) == '_entity_src_nat.details'
                        ) {
                            ...
                            prevLine.substr(0, 29) == '_pdbx_struct_oper_list.vector' &&
                            ...
                            prevLine.trim() == '_pdbx_poly_seq_scheic.hetero' ||
                            (bStartResidues && !bResidues)
                        ) {
                    ...
                    if (type === 'mmtfid' && missingseq !== undefined) {
                        ...
                        for (let i = 0, il = missingseq.length; i < il; ++i) {
                            ...
                            if (chainMissingResidueArray[chainNum] === undefined)
                                chainMissingResidueArray[chainNum] = []
                            let resObject = {}
                            resObject.resi = resi
    ...
    loadMmcifOpmDataPart2(data, pdbid) {
        let ic = this.icn3d,
            me = ic.icn3dui
