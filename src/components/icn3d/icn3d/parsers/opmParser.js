...
class OpmParser {
    ...
    downloadOpm(opmid) {
        ...
        $.ajax({
            ...
            error: function (xhr, textStatus, errorThrown) {
                ...
                alert(
                    'This is probably not a transmembrane protein. It has no data in Orientations of Proteins in Membranes(OPM) database.',
                )
                return
            },
        })
    ...
    loadOpmData(data, pdbid, bFull, type, pdbid2) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        let url, dataType
        ...
        url =
            'https://www.ncbi.nlm.nih.gov/Structure/mmdb/mmdb_strview.cgi?v=2&program=icn3d&opm&uid=' +
            pdbid.toLowerCase()
        dataType = 'jsonp'
    ...
    parseAtomData(data, pdbid, bFull, type, pdbid2) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (type === 'mmtf') {
