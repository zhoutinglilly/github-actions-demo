...
class XyzParser {
    ...
    setXyzAtomSeq(AtomHash, moleculeNum, chainNum, residueNum) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        for (let j = 0, jl = serialArray.length; j < jl; ++j) {
            ...
            for (let k = j + 1, kl = serialArray.length; k < kl; ++k) {
                ...
                let maxR =
                    1.2 *
                    (me.parasCls.covalentRadii[atom0.elem.toUpperCase()] +
                        me.parasCls.covalentRadii[atom1.elem.toUpperCase()])
