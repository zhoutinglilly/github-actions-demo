...
class RealignParser {
    ...
    realign() {
        ...
        for (let serial in ic.hAtoms) {
            ...
            if (
                (ic.proteins.hasOwnProperty(serial) && atom.name == 'CA') ||
                ...
                ic.realignResid[atom.structure].push({
                    resid: atom.structure + '_' + atom.chain + '_' + atom.resi,
                    resn: me.utilsCls.residueName2Abbr(atom.resn.substr(0, 3)).substr(0, 1),
                })
    ...
    parseChainRealignPredefined(chainidArray, struct2SeqHash, struct2CoorHash, struct2resid) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        for (let index = 0, indexl = chainidArray.length - 1; index < indexl; ++index) {
            ...
            let hAtomsTmp = ic.ParserUtilsCls.alignCoords(
                coord2,
                coord1,
                fromStruct,
                undefined,
    ...
    parseChainRealignData(
        dataArray,
        chainresiCalphaHash2,
        chainidArray,
        struct2SeqHash,
        struct2CoorHash,
        struct2resid,
            ...
            if (query !== undefined && target !== undefined) {
                ...
                for (let i = 0, il = segArray.length; i < il; ++i) {
                    ...
                    for (let j = 0; j <= seg.orito - seg.orifrom; ++j) {
                        let chainid1 = residArray1[j + seg.orifrom].substr(
                            0,
                            residArray1[j + seg.orifrom].lastIndexOf('_'),
                        )
                        ...
                        if (
                            j == 0 ||
                            (prevChain1 == chainid1 && prevChain2 == chainid2) ||
                            (prevChain1 != chainid1 && prevChain2 != chainid2)
                        ) {
                            ic.realignResid[toStruct].push({
                                resid: residArray1[j + seg.orifrom],
                                resn: seq1[j + seg.orifrom],
                            })
                            ic.realignResid[fromStruct].push({
                                resid: residArray2[j + seg.from],
                                resn: seq2[j + seg.from],
                            })
                        }
    ...
    realignChainOnSeqAlign(chainresiCalphaHash2, chainidArray, bRealign, bPredefined) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
                ...
                for (let j = 0, jl = resiArray.length; j < jl; ++j) {
                    if (resiArray[j].indexOf('-') != -1) {
                        ...
                        for (let k = parseInt(startEnd[0]); k <= parseInt(startEnd[1]); ++k) {
                            ...
                            if (
                                !ic.chainsSeq[chainid][k - base] ||
                                me.parasCls.b62ResArray.indexOf(ic.chainsSeq[chainid][k - base].name.toUpperCase()) ==
                                    -1
                            )
                                continue
                            ...
                            for (let serial in ic.residues[chainid + '_' + k]) {
                                let atom = ic.atoms[serial]
                                if (
                                    (ic.proteins.hasOwnProperty(serial) && atom.name == 'CA' && atom.elem == 'C') ||
                                    (ic.nucleotides.hasOwnProperty(serial) &&
                                        (atom.name == "O3'" || atom.name == 'O3*') &&
                                        atom.elem == 'O')
                                ) {
        ...
        if (bPredefined) {
            ...
            $.when
                ...
                .then(function () {
                    ...
                    thisClass.parseChainRealignData(
