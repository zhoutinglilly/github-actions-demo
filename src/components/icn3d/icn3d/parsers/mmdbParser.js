...
class MmdbParser {
    ...
    parseMmdbDataPart1(data, type) {
        ...
        if (type === 'query') {
            ...
            ic.interactionData = {
                moleculeInfor: data.moleculeInfor,
                intrac: data.intrac,
                intracResidues: data.intracResidues,
            }
        }
        ...
        for (let i in molid2rescount) {
            ...
            let color =
                molid2rescount[i].color === undefined
                    ? '#CCCCCC'
                    : '#' + ('000000' + molid2rescount[i].color.toString(16)).slice(-6)
    ...
    parseMmdbData(data, type, chainid, chainIndex, bLastQuery) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (type === undefined) {
            ...
            let hAtoms = ic.loadAtomDataCls.loadAtomDataIn(
                data,
                id,
                'mmdbid',
                undefined,
    ...
    downloadMmdbPart2(type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (ic.bAssemblyUseAsu) {
            // set up symmetric matrices
    ...
    downloadBlast_rep_id(sequence_structure_ids) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ic.bCid = undefined
    ...
    loadMmdbOpmData(data, pdbid, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    loadMmdbOpmDataPart2(data, pdbid, type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    downloadUniprotid(uniprotid) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                let redirectUrl =
                    'https://www.ncbi.nlm.nih.gov/structure?linkname=protein_structure&from_uid=' + giArray.join(',')
