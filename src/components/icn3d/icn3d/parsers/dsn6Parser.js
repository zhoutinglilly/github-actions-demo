...
class Dsn6Parser {
    ...
    loadDsn6Data(dsn6data, type, sigma) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        let data = new Float32Array(header.xExtent * header.yExtent * header.zExtent)
    ...
    getMatrix(header) {
        ...
        let basisX = [h.xlen, 0, 0]

        let basisY = [h.ylen * Math.cos((Math.PI / 180.0) * h.gamma), h.ylen * Math.sin((Math.PI / 180.0) * h.gamma), 0]
        ...
        basisZ[2] = Math.sqrt(
            h.zlen * h.zlen * Math.sin((Math.PI / 180.0) * h.beta) * Math.sin((Math.PI / 180.0) * h.beta) -
                basisZ[1] * basisZ[1],
        )
        ...
        matrix.multiply(new THREE.Matrix4().makeTranslation(h.xStart, h.yStart, h.zStart))
    ...
    loadDsn6FileUrl(type) {
        ...
        if (!url) {
            ...
            me.htmlCls.clickMenuCls.setLogCmd(
                'set map ' + type + ' sigma ' + sigma + ' | ' + encodeURIComponent(url),
                true,
            )
        }
    }
