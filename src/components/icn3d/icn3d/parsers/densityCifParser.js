...
class DensityCifParser {
    ...
    densityCifParser(pdbid, type, sigma, emd) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    parseChannels(densitydata, type, sigma) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        if (type == '2fofc' || type == 'fofc') {
            ...
            let header = {
                xExtent: sampleCount[0],
                yExtent: sampleCount[1],
                zExtent: sampleCount[2],
                mean: density.valuesInfo.mean,
                sigma: density.valuesInfo.sigma,
            }
            ...
            let header = {
                ...
                yExtent: sampleCount[1],
                zExtent: sampleCount[2],
                max: density.valuesInfo.max,
                min: density.valuesInfo.min,
            }
    ...
    CIFParse(block) {
        ...
        function readValues(col, xyzSampleCount, sampleCount, axisIndices) {
            ...
            let coord = [0, 0, 0]
            let iX = axisIndices[0],
                iY = axisIndices[1],
                iZ = axisIndices[2]
            let mX = sampleCount[0],
                mY = sampleCount[1],
                mZ = sampleCount[2]
        ...
        function createSpacegroup(number, size, angles) {
            let alpha = (Math.PI / 180.0) * angles[0],
    ...
    BinaryParse(data) {
        ...
        let Category = (function () {
            ...
            Category.prototype.toJSON = function () {
                let _this = this
                let rows = []
                let columns = this.columnNameList.map(function (name) {
        ...
        function wrapColumn(column) {
            if (!column.data.data) return _UndefinedColumn
            ...
            for (let i = offset, end = offset + length; i < end; i++) {
                ...
                if ((byte & 0x80) === 0x00) {
                    ...
                    chunk[chunkOffset++] = String.fromCharCode(
                        ((byte & 0x0f) << 12) | ((data[++i] & 0x3f) << 6) | ((data[++i] & 0x3f) << 0),
                    )
