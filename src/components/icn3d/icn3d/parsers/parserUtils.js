...
class ParserUtils {
    ...
    alignCoords(coordsFrom, coordsTo, secondStruct, bKeepSeq, chainid_t, chainid, chainIndex, bChainAlign) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    getMissingResidues(seqArray, type, chainid) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    set2DDiagramsForAlign(mmdbid1, mmdbid2) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    set2DDiagramsForChainalign(chainidArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    parse2DDiagramsData(dataArray, chainidArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    download2Ddgm(mmdbid, structureIndex) {
        let me = this
        ;('use strict')
    ...
    transformToOpmOriForAlign(pdbid, chainresiCalphaHash2, bResi_ori) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (chainresiCalphaHash2 !== undefined) {
            ...
            let bOneChain =
                Object.keys(chainresiCalphaHash1.chainresiCalphaHash).length == 1 ||
                Object.keys(chainresiCalphaHash2.chainresiCalphaHash).length == 1
                    ? true
                    : false
    ...
    addOneDumAtom(pdbid, atomName, x, y, z, lastSerial) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let resn = 'DUM'
        let chain = 'MEM'
    ...
    renderStructure() {
        ...
        if (ic.bInitial) {
            ...
            if (ic.bOpm && (me.cfg.align !== undefined || me.cfg.chainalign !== undefined)) {
                // show membrane
    ...
    getMassCenter(psum, cnt) {
        let ic = this.icn3d,
            me = ic.icn3dui
        return psum.multiplyScalar(1.0 / cnt)
    }

    getGeoCenter(pmin, pmax) {
        let ic = this.icn3d,
            me = ic.icn3dui
        return pmin.clone().add(pmax).multiplyScalar(0.5)
    }

    getStructureSize(atoms, pmin, pmax, center) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let maxD = 0
        for (let i in atoms) {
            ...
            if (
                Math.round(pmin.x) == Math.round(coord.x) ||
                Math.round(pmin.y) == Math.round(coord.y) ||
                Math.round(pmin.z) == Math.round(coord.z) ||
                Math.round(pmax.x) == Math.round(coord.x) ||
                Math.round(pmax.y) == Math.round(coord.y) ||
                Math.round(pmax.z) == Math.round(coord.z)
            ) {
