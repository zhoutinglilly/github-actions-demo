...
class ApplyOther {
    ...
    applyChemicalbindingOptions(options) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (options === undefined) options = ic.opts
        ...
        if (options.chemicalbinding === 'show') {
            let startAtoms
            if (ic.chemicals !== undefined && Object.keys(ic.chemicals).length > 0) {
                // show chemical-protein interaction
        ...
        if (!ic.proteins.hasOwnProperty(serial) && !ic.nucleotides.hasOwnProperty(serial)) {
            ...
            for (let i in ic.residues[residueid]) {
                ...
                if (atom.name === 'N3') {
                    ...
                    break
                } else if (atom.name === 'CA' && atom.ss == 'coil') {
                    // protein coil: CA
                    pos = ic.atoms[i].coord
                    break
                } else if (atom.name === 'CA' && (atom.ss == 'helix' || atom.ss == 'sheet')) {
                    // protein secondary: CA
