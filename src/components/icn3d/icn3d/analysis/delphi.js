...
class Delphi {
    ...
    CalcPhiUrl(gsize, salt, contour, bSurface, url) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    getPdbStr(bNode) {
        ...
        if (atomCnt > 30000) {
            if (!bNode) {
                ...
                console.log(
                    'The maximum number of allowed atoms is 30,000. Please try it again with selected chains...',
                )
            }
    ...
    CalcPhi(gsize, salt, contour, bSurface, data) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    PhiParser(url, type, contour, bSurface) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        //var dataType;
    ...
    */
    ...
    loadPhiData(data, contour, bSurface) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        matrix.identity()
        matrix.multiply(new THREE.Matrix4().makeTranslation(header.ori.x, header.ori.y, header.ori.z))
        ic.mapData.matrixPhi = matrix
    }

    loadCubeData(data, contour, bSurface) {
        let ic = this.icn3d,
            me = ic.icn3dui
        // http://compbio.clemson.edu/downloadDir/delphi/delphi_manual8.pdf
    ...
    */
    ...
    applyCommandPhi(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        // chain functions together
        ic.deferredPhi = $.Deferred(function () {
    ...
    applyCommandDelphi(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this

        // chain functions together
        ic.deferredDelphi = $.Deferred(function () {
            let ic = thisClass.icn3d
    ...
    loadDelphiFile(type) {
        ...
        let contour =
            type == 'delphi2' ? $('#' + ic.pre + 'delphicontour2').val() : $('#' + ic.pre + 'delphicontour').val()
        ...
        if (bSurface) {
            ...
            me.htmlCls.clickMenuCls.setLogCmd(
                'set delphi ' + displayType + ' | contour ' + contour + ' | gsize ' + gsize + ' | salt ' + salt,
                true,
            )
        }
    }
    ...
    loadPhiFile(type) {
        ...
        let contour =
            type == 'pqr' || type == 'phi' || type == 'cube'
                ? $('#' + ic.pre + 'phicontour').val()
                : $('#' + ic.pre + 'phicontour2').val()
        if (!file) {
    ...
    loadPhiFileUrl(type) {
        ...
        let contour =
            type == 'pqrurl' || type == 'phiurl' || type == 'cubeurl'
                ? $('#' + ic.pre + 'phiurlcontour').val()
                : $('#' + ic.pre + 'phiurlcontour2').val()
        if (!url) {
