...
class ApplySymd {
    ...
    applySymmetry(title, bSymd, inDataArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        for (let i = 0, il = dataArray.length; i < il; ++i) {
            ...
            if (symmetryType == 'C' || (symmetryType == 'D' && order == nSide)) {
                ...
                if (!bSymd) {
                    ...
                    let resid =
                        ic.atoms[lastSerial].structure +
                        '_' +
                        ic.atoms[lastSerial].chain +
                        '_' +
                        ic.atoms[lastSerial].resi
                ...
                if (startInit && startPrev)
                    ic.cylinderCls.createCylinder(startInit, startPrev, polygonRadius, colorPolygon, 0)
                if (endInit && endPrev) ic.cylinderCls.createCylinder(endInit, endPrev, polygonRadius, colorPolygon, 0)
            } else if (
                (symmetryType == 'T' && order == 3) ||
