...
class Scap {
    ...
    adjust2DWidth(id) {
        ...
        let height = $('#' + ic.pre + 'dl_selectannotations').hasClass('ui-dialog-content')
            ? $('#' + ic.pre + 'dl_selectannotations').dialog('option', 'height')
            : me.htmlCls.HEIGHT
        let width = $('#' + ic.pre + 'dl_selectannotations').hasClass('ui-dialog-content')
            ? halfWidth * 2
            : me.htmlCls.WIDTH * 0.5

        $('#' + id).dialog('option', 'width', width)
        $('#' + id).dialog('option', 'height', height)
        let position = {
            my: 'left-' + halfWidth + ' top+' + me.htmlCls.MENU_HEIGHT,
            at: 'right top',
            of: '#' + ic.pre + 'viewer',
            collision: 'none',
        }
    ...
    retrieveScap(snp, bInteraction, bPdb) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
