...
class Cartoon2d {
    ...
    draw2Dcartoon_base(type, bResize) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
    ...
    getCartoonSvg(type, graphStr) {
        ...
        for (let i = 0, il = graph.links.length; i < il; ++i) {
            ...
            let x1 = ic.ctnNodeHash[id1].x,
                y1 = me.htmlCls.width2d - ic.ctnNodeHash[id1].y,
                x2 = ic.ctnNodeHash[id2].x,
                y2 = me.htmlCls.width2d - ic.ctnNodeHash[id2].y

            if (type == 'chain') {
                html +=
                    "<g class='icn3d-ctinteraction' chainid1='" +
                    ic.ctnNodeHash[id1].id +
                    "' chainid2='" +
                    ic.ctnNodeHash[id2].id +
                    "' >"
            } else if (type == 'domain') {
                html +=
                    "<g class='icn3d-ctinteraction' from1='" +
                    ic.ctnNodeHash[id1].from +
                    "' to1='" +
                    ic.ctnNodeHash[id1].to +
                    "' from2='" +
                    ic.ctnNodeHash[id2].from +
                    "' to2='" +
                    ic.ctnNodeHash[id2].to +
                    "' >"
            } else if (type == 'secondary') {
                {
                    /* eslint-disable */
                }
                ;(x1 = ic.ctnNodeHash[id1].x2),
                    (y1 = me.htmlCls.width2d - ic.ctnNodeHash[id1].y2),
                    (x2 = ic.ctnNodeHash[id2].x1),
                    (y2 = me.htmlCls.width2d - ic.ctnNodeHash[id2].y1)

                html +=
                    "<g class='icn3d-ctinteraction' range1='" +
                    ic.ctnNodeHash[id1].range +
                    "' range2='" +
                    ic.ctnNodeHash[id2].range +
                    "' >"
            }
    ...
    drawHelix(type, id, ss, x, y, x1, y1, x2, y2, length, angle, color) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        html +=
            ...
            "' style='fill:" +
            textcolor +
            "; text-anchor:middle' class='icn3d-node-text8' >" +
            idStr +
            '</text>'
    ...
    drawOval(type, id, x, y, rx, ry, angle, color, from, to) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let strokecolor = 'none'
        let strokewidth = '1'
        ...
        let html = type == 'chain' ? "<g chainid='" + id + "' >" : "<g from='" + from + "' to='" + to + "' >"
        html += '<title>' + type + ' ' + idStr + '</title>'
        ...
        html +=
            ...
            "' style='fill:" +
            textcolor +
            "; text-anchor:middle' class='icn3d-node-text12' >" +
            idStr +
            '</text>'
    ...
    getCartoonData(type, node_link) {
        ...
        resStr +=
            linkStr + disulfideLinkStr + crossLinkStr + contactLinkStr + hBondLinkStr + ionicLinkStr + halogenpiLinkStr
    ...
    getNodesLinksForSetCartoonBase(type) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
        ...
        if (type == 'chain') {
            ...
            for (let chainid in chainidHash) {
                ...
                let angle =
                    (new THREE.Vector2(
                        center_x_y_z[1].x - center_x_y_z[0].x,
                        center_x_y_z[1].y - center_x_y_z[0].y,
                    ).angle() *
                        180) /
                    Math.PI
            ...
            for (let i = 0, il = itemArray.length; i < il; ++i) {
                ...
                let x =
                    rangeX < 1
                        ? 0.5 * me.htmlCls.width2d
                        : ((item.x - minX) / rangeX) * (me.htmlCls.width2d - 2 * offset) + offset
                let y =
                    rangeY < 1
                        ? 0.5 * me.htmlCls.width2d
                        : ((item.y - minY) / rangeY) * (me.htmlCls.width2d - 2 * offset) + offset
                ...
                nodeArray.push(
                    '{"id": "' +
                        ...
                        item.ry.toFixed(0) +
                        ', "ang": ' +
                        item.ang.toFixed(0) + //+ ', "shape": ' + shapeid
                        ', "c": "' +
                        item.c.toUpperCase() +
                        '"}',
                )
                        ...
                        if (cnt > 0 && prevChain == atom.chain) {
                            linkArray.push(
                                '{"source": "' +
                                    ...
                                    thickness +
                                    ', "c": "' +
                                    prevAtom.color.getHexString().toUpperCase() +
                                    '"}',
                            )
                        }
            ...
            for (let i = 0, il = itemArray.length; i < il; ++i) {
                ...
                let x1 =
                    rangeX < 1
                        ? 0.5 * me.htmlCls.width2d
                        : ((item.x1 - minX) / rangeX) * (me.htmlCls.width2d - 2 * offset) + offset
                let y1 =
                    rangeY < 1
                        ? 0.5 * me.htmlCls.width2d
                        : ((item.y1 - minY) / rangeY) * (me.htmlCls.width2d - 2 * offset) + offset
                let x2 =
                    rangeX < 1
                        ? 0.5 * me.htmlCls.width2d
                        : ((item.x2 - minX) / rangeX) * (me.htmlCls.width2d - 2 * offset) + offset
                let y2 =
                    rangeY < 1
                        ? 0.5 * me.htmlCls.width2d
                        : ((item.y2 - minY) / rangeY) * (me.htmlCls.width2d - 2 * offset) + offset
    ...
    getNodesLinksForDomains(chainid2pssmid) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let nodeArray = [],
            linkArray = []
    ...
    click2Dcartoon() {
        ...
        me.myEventCls.onIds('#' + me.pre + '2dctn_chain', 'click', function (e) {
            let ic = me.icn3d
            e.preventDefault()
        ...
        me.myEventCls.onIds('#' + me.pre + '2dctn_domain', 'click', function (e) {
            let ic = me.icn3d
            e.preventDefault()
            thisClass.initCartoonSvg()
        ...
        me.myEventCls.onIds('#' + me.pre + '2dctn_secondary', 'click', function (e) {
            let ic = me.icn3d
            e.preventDefault()
            thisClass.initCartoonSvg()
        ...
        $(document).on('click', '#' + ic.pre + 'dl_2dctn .icn3d-ctnode', function (e) {
            ...
            let select =
                type == 'chain' ? 'select chain ' + chainid : 'select ' + ic.resid2specCls.residueids2spec(residArray)
