...
class Diagram2d {
    ...
    draw2Ddgm(data, mmdbid, structureIndex, bUpdate) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        for (let molid in data.moleculeInfor) {
            ...
            let chainNameFinal =
                chainNameHash[chainName] === 1 ? chainName : chainName + chainNameHash[chainName].toString()
        ...
        if (bUpdate === undefined || !bUpdate) {
            for (let i = 0, il = data['intracResidues'].length; i < il; ++i) {
                ...
                for (let molid in pair) {
                    ...
                    let type2
                    if (ic.chemicals.hasOwnProperty(atom2.serial)) {
                        // 1. chemical interacting with proteins
                        type2 = 'chemical'
                    } else if (ic.nucleotides.hasOwnProperty(atom2.serial)) {
                        // 2. DNA interacting with proteins
                        type2 = 'nucleotide'
                    } else if (ic.ions.hasOwnProperty(atom2.serial)) {
                        // 3. ions interacting with proteins
                        type2 = 'ion'
                    } else if (ic.proteins.hasOwnProperty(atom2.serial)) {
                        // 4. protein interacting with proteins
                        type2 = 'protein'
                    } else if (ic.water.hasOwnProperty(atom2.serial)) {
                        // 5. water interacting with proteins
                        type2 = 'water'
                    }
        ...
        for (let index = 0, indexl = allMolidArray.length; index < indexl; ++index) {
            ...
            if (missingMolidArray.indexOf(molid) === -1) {
                ...
                if (dgm.shape === 'rect') {
                    ...
                    nodeHtml += this.draw2DNucleotide(
                        x + 0.5 * width,
                        y + 0.5 * height,
                        chainid,
                        oriChain,
                    ...
                    chemNodeHtml += this.draw2DChemical(
                        x,
                        y,
                        chainid,
                        oriChain,
                ...
                let xCenter, yCenter
                if (missingMolid2intrac[molid] !== undefined && missingMolid2intrac[molid].length > 1) {
                    // has interactions
                    // find its position
                    let xSum = 0,
                        ySum = 0
                    ...
                    // has NO interactions or just one interaction
        ...
        for (let i = 0, il = lines.length; i < il; ++i) {
            ...
            // if redraw2d diagram and the molid is not displayed, skip
            if (bUpdate && (!displayedMolids.hasOwnProperty(pair[0]) || !displayedMolids.hasOwnProperty(pair[1])))
                continue
    ...
    set2DdgmNote(bAlign) {
        ...
        if (me.utilsCls.isMac()) {
            ...
            html +=
                "<span style='margin-right:18px;display: inline-block;transform: skew(-25deg);'>&#9634;</span>Biopolymer<br>"
        } else {
    ...
    highlightNode(type, highlight, base, ratio) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (ratio < 0.2) ratio = 0.2
        ...
        if (type === 'rect') {
            ...
            $(highlight).attr(
                'points',
                (x + x0diff * ratio).toString() +
                    ', ' +
                    (y + y0diff * ratio).toString() +
                    ', ' +
                    (x + x1diff * ratio).toString() +
                    ', ' +
                    (y + y1diff * ratio).toString() +
                    ', ' +
                    (x + x2diff * ratio).toString() +
                    ', ' +
                    (y + y2diff * ratio).toString() +
                    ', ' +
                    (x + x3diff * ratio).toString() +
                    ', ' +
                    (y + y3diff * ratio).toString(),
            )
        }
    }

    removeLineGraphSelection() {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    click2Ddgm() {
        ...
        $(document).on('click', '#' + ic.pre + 'dl_2ddgm .icn3d-node', function (e) {
            ...
            let ratio = 1.0
            if (ic.alnChains[chainid] !== undefined)
                ratio = (1.0 * Object.keys(ic.alnChains[chainid]).length) / Object.keys(ic.chains[chainid]).length
        ...
        $(document).on('click', '#' + ic.pre + 'dl_2ddgm .icn3d-interaction', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            if (Object.keys(ic.hAtoms).length < Object.keys(ic.atoms).length) ic.definedSetsCls.setMode('selection')
        ...
        $(document).on('click', '#' + ic.pre + 'dl_linegraph .icn3d-node', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            if (Object.keys(ic.hAtoms).length < Object.keys(ic.atoms).length) ic.definedSetsCls.setMode('selection')
        ...
        $(document).on('click', '#' + ic.pre + 'dl_scatterplot .icn3d-node', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
...
*/
        ...
        $(document).on('click', '#' + ic.pre + 'dl_linegraph .icn3d-interaction', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
            if (Object.keys(ic.hAtoms).length < Object.keys(ic.atoms).length) ic.definedSetsCls.setMode('selection')
        ...
        $(document).on('click', '#' + ic.pre + 'dl_scatterplot .icn3d-interaction', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
...
*/
        ...
        $(document).on('click', '#' + ic.pre + 'dl_contactmap .icn3d-interaction', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
...
*/
        ...
        $(document).on('click', '#' + ic.pre + 'dl_contactmap .icn3d-node', function (e) {
            let ic = thisClass.icn3d
            e.stopImmediatePropagation()
...
*/
    ...
    selectInteraction(chainid1, chainid2) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    selectInteractionAtoms(chainid1, chainid2) {
        let ic = this.icn3d,
    ...
    draw2DProtein(x, y, chainid, oriChain, chain, chainname, alignNum, color, oricolor, factor, ratio) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let strokecolor = '#000000'
        let strokewidth = '1'
        ...
        html +=
            "<circle class='icn3d-basenode' cx='" +
            x +
            "' cy='" +
            y +
            "' r='" +
            ...
            "' stroke='" +
            strokecolor +
            "' class='icn3d-node' chainid='" +
            chainid +
            "' />"

        html +=
            "<circle class='icn3d-hlnode' cx='" +
            x +
            "' cy='" +
            y +
            "' r='" +
        ...
        if (alignNum !== '')
            html +=
                ...
                (x - adjustx).toString() +
                "' y='" +
                (y + r + adjusty + halfLetHigh).toString() +
                "' style='fill:" +
                oricolor +
                '; font-size:' +
                smallfontsize +
    ...
    draw2DNucleotide(x, y, chainid, oriChain, chain, chainname, alignNum, color, oricolor, factor, ratio) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let strokecolor = '#000000'
        let strokewidth = '1'
        ...
        // highlight
        html +=
            "<rect class='icn3d-hlnode' x='" +
            (x + (width / 2.0) * (1 - ratio)).toString() +
            "' y='" +
            (y + (height / 2.0) * (1 - ratio)).toString() +
            "' width='" +
        ...
        html +=
            "<text x='" +
            (x + width / 2 - adjustx).toString() +
            "' y='" +
            (y + height / 2 + adjusty).toString() +
            "' style='fill:" +
            textcolor +
            '; font-size:' +
            fontsize +
        ...
        if (alignNum !== '')
            html +=
                ...
                (x + width / 2 - adjustx).toString() +
                "' y='" +
                (y + height + adjusty + halfLetHigh).toString() +
                "' style='fill:" +
                oricolor +
                '; font-size:' +
                smallfontsize +
    ...
    draw2DChemical(x, y, chainid, oriChain, chain, chainname, alignNum, color, oricolor, factor, ratio, bBiopolymer) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let strokecolor = '#000000'
        let strokewidth = '1'
        ...
        html += '<title>Chain ' + oriChain + ': ' + chainname + '</title>'
        html +=
            "<polygon class='icn3d-basenode' points='" +
            x0 +
            ', ' +
            y0 +
            ',' +
        ...
        html +=
            "<polygon class='icn3d-hlnode' points='" +
            (x + x0diff * ratio).toString() +
            ', ' +
            (y + y0diff * ratio).toString() +
            ',' +
        ...
        html +=
            "<text x='" +
            (x + smalladjustx).toString() +
            "' y='" +
            (y + smalladjusty).toString() +
            "' style='fill:" +
            textcolor +
            '; font-size:' +
            smallfontsize +
        ...
        if (alignNum !== '')
            html +=
                ...
                (x + smalladjustx).toString() +
                "' y='" +
                (y + smalladjusty + halfLetHigh).toString() +
                "' style='fill:" +
                oricolor +
                '; font-size:' +
                smallfontsize +
