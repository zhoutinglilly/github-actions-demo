...
class Symd {
    ...
    applyCommandSymdBase(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        this.retrieveSymd()
    }

    applyCommandSymd(command) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let thisClass = this
            ...
            success: function (data) {
                ...
                if (symmetryArray !== undefined) {
                    ...
                    for (let i = 0, il = symmetryArray.length; i < il; ++i) {
                        ...
                        for (let j = 0, jl = rotation_axes.length; j < jl; ++j) {
                            ...
                            let start = new THREE.Vector3(
                                rotation_axes[j].start[0],
                                rotation_axes[j].start[1],
                                rotation_axes[j].start[2],
                            )
                            let end = new THREE.Vector3(
                                rotation_axes[j].end[0],
                                rotation_axes[j].end[1],
                                rotation_axes[j].end[2],
                            )
                    ...
                    if (ic.symdArray.length == 0) {
                        $('#' + ic.pre + 'dl_symd').html(
                            '<br>The selected residues have no detected symmetry with a Z score of ' +
                                data.zscore +
                                " from the program <a href='https://symd.nci.nih.gov/' target='_blank'>SymD</a>.",
                        )
                        ...
                        let residArrayHashFinal1 = {},
                            residArrayHashFinal2 = {}
                        let residArrayFinal1 = [],
                            residArrayFinal2 = []
                        ...
                        if (Object.keys(chainCntHash).length == 1) {
                            ...
                            let residArrayFromHash1 = Object.keys(residArrayHash1),
                                residArrayFromHash2 = Object.keys(residArrayHash2)
                            for (let i = 0; i < nResUnit; ++i) {
                                if (!residArrayHashFinal1.hasOwnProperty(residArrayFromHash2[i])) {
                                    // do not appear in both original and permuted
                            ...
                            for (let i = 0, il = chainCntHash[selChainid].length; i < il; ++i) {
                                ...
                                if (!residArrayHashFinal1.hasOwnProperty(residArrayFromHash2[pos])) {
                                    // do not appear in both original and permuted
                        ...
                        html +=
                            'The symmetry ' +
                            symmetryArray[0].symbol +
                            " was calculated dynamically using the program <a href='https://symd.nci.nih.gov/' target='_blank'>SymD</a>. The Z score " +
                            data.zscore +
                            ' is greater than the threshold Z score 8. The RMSD is ' +
                            rmsd +
                            ' angstrom. <br><br>The following sequence alignment shows the residue mapping of the best aligned sets: "symOri" and "symPerm", which are also available in the menu "Analysis > Defined Sets".<br>'
                        ...
                        me.htmlCls.clickMenuCls.setLogCmd(
                            'select ' +
                                ic.resid2specCls.residueids2spec(Object.keys(residArrayHashFinal1)) +
                                ' | name ' +
                                name,
                            false,
                        )
                        ...
                        me.htmlCls.clickMenuCls.setLogCmd(
                            'select ' +
                                ic.resid2specCls.residueids2spec(Object.keys(residArrayHashFinal2)) +
                                ' | name ' +
                                name,
                            false,
                        )
    ...
    setSeqAlignForSymmetry(residArray1, residArray2, bOnechain) {
        let ic = this.icn3d,
            me = ic.icn3dui
        //loadSeqAlignment
        let alignedAtoms = {}
        ...
        for (let i = 0, il = residArray1.length; i < il; ++i) {
            // K $1KQ2.A:2
