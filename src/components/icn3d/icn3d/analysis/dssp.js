...
class Dssp {
    ...
    parseDsspData(dataArray, struArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
                    ...
                    for (let i = 0, il = residueObjectArray.length; i < il; ++i) {
                        ...
                        if (ss !== prevSS) {
                            if (prevSS === 'coil') {
                                ...
                                (prevSS === 'sheet' && ss === 'helix') ||
                                (prevSS === 'helix' && ss === 'sheet')
                            ) {
                                //bSetPrevResidue = 1;
                                bSetPrevResidue = 2
                                ssbegin = true
                        ...
                        if (bSetPrevResidue == 1) {
                            //1: reset previous residue to "ssbegin = true"
                        ...
                        } else if (bSetPrevResidue == 2) {
                            //2: reset previous residue to "ssend = true"
                            let prevResid = chainNum + '_' + prevResi //(resi - 1).toString();
                            for (let j in ic.residues[prevResid]) {
