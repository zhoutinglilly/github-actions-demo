...
class ThreeDPrint {
    ...
    removeOneStabilizer(rmLineArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let index
        for (let i = 0, il = ic.pairArray.length; i < il; i += 2) {
            ...
            if (rmLineArray != undefined) {
                for (let j = 0, jl = rmLineArray.length; j < jl; j += 2) {
                    ...
                    if (
                        (atom1.serial == atomb1.serial && atom2.serial == atomb2.serial) ||
        ...
        if (Object.keys(ic.dAtoms).length > 0) {
            ...
            for (let i = 0, il = chainArray.length; i < il; ++i) {
                ...
                for (let j = 0, jl = ic.chainsSeq[chainid].length; j < jl; ++j) {
                    ...
                    if (
                        ic.secondaries[residueid] == 'c' ||
                        ic.secondaries[residueid] == 'E' ||
                        ic.secondaries[residueid] == 'H'
                    ) {
            ...
            for (let i = 0, il = residueArray.length; i < il; ++i) {
                ...
                let sphere = ic.contactCls.getNeighboringAtoms(
                    dAtomsNotWater,
                ...
                let bProtein = false
                if (ic.proteins.hasOwnProperty(atomArray[0])) {
                    // protein
                    ...
                    for (let serial in sphere) {
                        ...
                        if (
                            (ss == 'c' && (atom.resi > resi + 1 || atom.resi < resi - 1)) ||
                ...
                if (sphereArray.length > 0 && atomArray.length > 0) {
                    if (bProtein) {
                        ...
                        for (let j = 0, jl = atomArray.length; j < jl; ++j) {
                            if (j % n == 0) {
                                // make one line for every other 10 atoms
    ...
    getResidueRepAtom(serial) {
        ...
        if (!ic.proteins.hasOwnProperty(serial) && !ic.nucleotides.hasOwnProperty(serial)) {
            ...
            for (let i in ic.residues[residueid]) {
                let atom = ic.atoms[i]
                if (atom.name === 'CA' || atom.name === 'N3') {
                    // protein: CA, nucleotide: N3
