...
class ShareLink {
    ...
    shareLink(bPngHtml, bPngOnly) {
        ...
        if (!bPngHtml) {
            if (ic.bInputfile && !ic.bInputUrlfile) {
                alert(
                    "Share Link does NOT work when the data is from custom files. Please save 'iCn3D PNG Image' in the File menu and open it in iCn3D.",
                )
                return
            }
            if (bTooLong) {
                alert(
                    "The url is more than 4000 characters and may not work. Please save 'iCn3D PNG Image' or 'State File' and open them in iCn3D.",
                )
                return
            }
        ...
        let fdlUrl =
            'https://firebasedynamiclinks.googleapis.com/v1/shortLinks?key=AIzaSyBxl9CgM0dY5lagHL4UOhEpLWE1fuwdnvc'
        $.ajax({
            url: fdlUrl,
            type: 'POST',
            ...
            success: function (data) {
                ...
                if (data.shortLink !== undefined) {
                    ...
                    if (bPngHtml) {
                        // save png and corresponding html
                        let strArray = shorturl.split('/')
                        let shortName = strArray[strArray.length - 1]
                        ic.saveFileCls.saveFile(inputid + '-' + shortName + '.png', 'png')
                        let text =
                            '<div style="float:left; border: solid 1px #0000ff; padding: 5px; margin: 10px; text-align:center;">'
                        text +=
                            '<a href="https://structure.ncbi.nlm.nih.gov/icn3d/share.html?' +
                            shortName +
                            '" target="_blank">'
                ...
                if (urlArray.length == 2)
                    shorturl = 'https://structure.ncbi.nlm.nih.gov/icn3d/share.html?' + urlArray[1]
    ...
    shareLinkUrl(bAllCommands) {
        ...
        if (!ic.bInputUrlfile) {
            let inparaArray =
                inparaWithoutCommand && inparaWithoutCommand.substr(1) ? inparaWithoutCommand.substr(1).split('&') : []
        ...
        for (let il = ic.commands.length; i < il; ++i) {
            ...
            if (
                ((prevCommandStr.indexOf('select saved atoms') !== -1 ||
                    prevCommandStr.indexOf('select sets') !== -1) &&
            ...
            // remove all "show selection" except the last one
                ...
                prevCommandStr == 'show selection' &&
                ic.commands.slice(i).toString().indexOf('show selection') != -1
            ) {
                // do nothing
