...
class SaveFile {
    ...
    getSvgXml(id) {
        ...
        let head =
            '<svg title="graph" version="1.1" xmlns:xl="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" xmlns:dc="http://purl.org/dc/elements/1.1/">'
    ...
    savePng(id, filename, width, height) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return ''
    ...
    getAtomPDB(atomHash, bPqr, bNoChem) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let pdbStr = ''
            ...
            if (atom.ssbegin) {
                if (atom.ss == 'helix') {
                    ...
                    if (bHelixEnd)
                        pdbStr +=
                            helixStr.padEnd(15, ' ') +
                            atom.resn.padStart(3, ' ') +
                            atom.chain.padStart(2, ' ') +
    ...
    */
            ...
            let resi = atom.resi
            if (!isNaN(resi) && atom.chain.length > 3 && !isNaN(atom.chain.substr(3))) {
    ...
    */
    ...
    showTitle() {
        ...
        if (ic.molTitle !== undefined && ic.molTitle !== '') {
            ...
            let titlelinkColor =
                ic.opts['background'] == 'white' || ic.opts['background'] == 'grey' ? 'black' : me.htmlCls.GREYD
    ...
    getLinkToStructureSummary(bLog) {
        ...
        if (ic.inputid === undefined) {
            ...
            if (idArray.length === 1) {
                url += ic.inputid
                if (bLog)
                    me.htmlCls.clickMenuCls.setLogCmd('link to Structure Summary ' + ic.inputid + ': ' + url, false)
            } else if (idArray.length === 2) {
                url += idArray[0] + ' OR ' + idArray[1]
                if (bLog)
                    me.htmlCls.clickMenuCls.setLogCmd(
                        'link to structures ' + idArray[0] + ' and ' + idArray[1] + ': ' + url,
                        false,
                    )
            }
        }
    ...
    setEntrezLinks(db) {
        ...
        if (structArray.length === 1) {
            ...
            me.htmlCls.clickMenuCls.setLogCmd(
                'Entrez ' + db + ' about PDB ' + structArray[0] + ' OR ' + structArray[1] + ': ' + url,
                false,
            )
            window.open(url, '_blank')
        }
