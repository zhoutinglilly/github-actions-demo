...
class Export3D {
    ...
    exportStlFile(postfix) {
        ...
        if (
            ...
            ic.biomtMatrices.length > 1 &&
            ic.bAssembly &&
            Object.keys(ic.dAtoms).length * ic.biomtMatrices.length > ic.maxAtoms3DMultiFile
        ) {
            alert(
                ic.biomtMatrices.length +
                    ' files will be generated for this assembly. Please merge these files using some software and 3D print the merged file.',
            )
    ...
    */
    ...
    saveStlFile(mat) {
        ...
        if (Object.keys(ic.dAtoms).length > 70000) {
            alert(
                'Please display a subset of the structure to export 3D files. Then merge the files for 3D printing...',
            )
            return ['']
        }
    ...
    updateArray(array, inArray, indexBase) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    processStlMeshGroup(mdl, blobArray, mat) {
        let ic = this.icn3d,
            me = ic.icn3dui
        for (let i = 0, il = mdl.children.length; i < il; ++i) {
            let mesh = mdl.children[i]
            ...
            for (let j = 0, jl = indexArray.length; j < jl; j += 3) {
                ...
                if (normal !== undefined) {
                    ...
                    stlArray = this.updateArray(
                        stlArray,
                        me.convertTypeCls.passFloat32([normal.x, normal.y, normal.z]),
                        index,
                    )
                    index += 12
                } else {
    ...
    processVrmlMeshGroup(mdl, vrmlStrArray, vertexCnt, mat) {
        let ic = this.icn3d,
            me = ic.icn3dui
        for (let i = 0, il = mdl.children.length; i < il; ++i) {
            let mesh = mdl.children[i]
            ...
            for (let j = 0, jl = positionArray.length; j < jl; j += 3) {
                ...
                vrmlStrArray.push(
                    vertex.x.toPrecision(5) + ' ' + vertex.y.toPrecision(5) + ' ' + vertex.z.toPrecision(5),
                )
                vertex = undefined
