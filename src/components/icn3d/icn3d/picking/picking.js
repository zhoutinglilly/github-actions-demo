    ...
    showPicking(atom, x, y) {
        ...
        if (ic.pk != 0) {
            if (x !== undefined && y !== undefined) {
                ...
                if (ic.bAddCommands) {
                    ic.commands.push(
                        'pickatom ' + atom.serial + '|||' + ic.transformCls.getTransformationStr(transformation),
                    )
    ...
    showPickingHilight(atom) {
        ...
        let intersectAtoms =
            Object.keys(ic.hAtoms).length == Object.keys(ic.atoms).length
                ? {}
                : me.hashUtilsCls.intHash(ic.hAtoms, ic.pickedAtomList)
    ...
    selectStrandHelixFromAtom(atom) {
        ...
        if (!firstAtom.ssbegin && !isNaN(firstAtom.resi)) {
            for (let i = firstAtom.resi - 1; i > 0; --i) {
                ...
                if (
                    (firstAtom.ss !== 'coil' && atom.ss === firstAtom.ss && atom.ssbegin) ||
            ...
            for (let i = beginResi; i <= firstAtom.resi; ++i) {
                let residueid = firstAtom.structure + '_' + firstAtom.chain + '_' + i
                atomsHash = me.hashUtilsCls.unionHash(
                    atomsHash,
        ...
        for (let i = parseInt(lastAtom.resi) + 1; i <= parseInt(endChainResi); ++i) {
            ...
            if (
                (lastAtom.ss !== 'coil' && atom.ss === lastAtom.ss && atom.ssend) ||
                (lastAtom.ss === 'coil' && atom.ss !== lastAtom.ss)
            ) {
