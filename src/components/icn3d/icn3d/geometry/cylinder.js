...
class Cylinder {
    ...
    createCylinder(p0, p1, radius, color, bHighlight, color2, bPicking, bGlycan) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
        ...
        if (bHighlight === 1) {
            ...
            mesh.matrix
                .multiply(new THREE.Matrix4().makeScale(radius, radius, p0.distanceTo(p1)))
    ...
    createCylinder_base(p0, p1, radius, color, bHighlight, color2, bPicking) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
    ...
    createCylinderCurve(atoms, atomNameArray, radius, bLines, bHighlight) {
        ...
        let atom,
            maxDistance = 8.0 // max residue-residue (or nucleitide-nucleitide) distance allowed
        ...
        for (i in atoms) {
            ...
            if (
                start !== null &&
                currentChain === atom.chain &&
                currentResi != atom.resi && //currentResi + 1 === atom.resi
