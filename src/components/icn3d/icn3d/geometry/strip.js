...
class Strip {
    ...
    createStrip(
        ...
        if (pntsCA && ic.bDoublecolor && !ic.bCalphaOnly) {
            ...
            let pnts_clrs = me.subdivideCls.subdivide(
                pntsCA,
                colors,
                div,
                bShowArray,
            ...
            let pnts_clrs2 = me.subdivideCls.subdivide(
                prevCOArray,
                colors,
                div,
                bShowArray,
            ...
            if (!bNoSmoothen) {
                ...
                let pnts_clrs0 = me.subdivideCls.subdivide(
                    p0,
                    colors,
                    div,
                    bShowArray,
                ...
                let pnts_clrs1 = me.subdivideCls.subdivide(
                    p1,
                    colors,
                    div,
                    bShowArray,
        ...
        if (bHighlight === 1) {
            ...
            if (positions !== undefined) {
                ...
                for (let i = 0, il = p0.length; i < il; ++i) {
                    ...
                    if (
                        (currPos !== prevPos && parseInt(currPos) !== parseInt(prevPos) + 1 && prevPos !== undefined) ||
                        i === il - 1
                    ) {
                        // first tube
    ...
    setCalphaDrawnCoord(pnts, div, calphaIdArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        let index = 0

        if (calphaIdArray !== undefined) {
            for (let i = 0, il = pnts.length; i < il; i += div) {
                // pnts.length = (calphaIdArray.length - 1) * div + 1
