...
class CartoonNucl {
    ...
    // modified from GLmol (http://webglmol.osdn.jp/index-en.html)
    drawStrandNucleicAcid(atomlist, num, div, fill, nucleicAcidWidth, thickness, bHighlight) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
        ...
        for (i in atomlist) {
            ...
            if (
                (atom.name === "O3'" || atom.name === 'OP2' || atom.name === 'O3*' || atom.name === 'O2P') &&
                !atom.het
            ) {
        ...
        if (fill) ic.stripCls.createStrip(pnts[0], pnts[1], colors, div, thickness, bHighlight)
        for (j = 0; !thickness && j < num; j++) ic.curveCls.createCurveSub(pnts[j], 1, colors, div, bHighlight)
    }
    ...
    drawNucleicAcidStick(atomlist, bHighlight) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
            ...
            if (atom.resi !== currentResi || atom.chain !== currentChain) {
                if (start !== null && end !== null) {
                    ic.cylinderCls.createCylinder(
                        new THREE.Vector3(start.coord.x, start.coord.y, start.coord.z),
                        new THREE.Vector3(end.coord.x, end.coord.y, end.coord.z),
                        ic.cylinderRadius,
                        start.color,
                        bHighlight,
                    )
