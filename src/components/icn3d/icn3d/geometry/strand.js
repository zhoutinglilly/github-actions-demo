...
class Strand {
    ...
    getSSExpandedAtoms(atoms, bHighlight) {
        ...
        for (let serial in atoms) {
            ...
            if (
                (currChain !== prevChain && prevChain !== undefined) ||
                ...
                if (!isNaN(firstAtom.resi) && firstAtom.ss !== 'coil' && !firstAtom.ssbegin) {
                    ...
                    for (let i = beginResi; i < firstAtom.resi; ++i) {
                        let residueid = firstAtom.structure + '_' + firstAtom.chain + '_' + i
                        atomsAdjust = me.hashUtilsCls.unionHash(
                            atomsAdjust,
                ...
                if (!isNaN(firstAtom.resi) && ic.pk === 3 && bHighlight === 1 && firstAtom.ss === 'coil') {
                    ...
                    if (ic.residues.hasOwnProperty(residueid)) {
                        ...
                        atoms = me.hashUtilsCls.unionHash(
                            atoms,
                            me.hashUtilsCls.hash2Atoms(ic.residues[residueid], ic.atoms),
                        )
                    }
                ...
                if (lastAtom.ss !== undefined && lastAtom.ss !== 'coil' && !lastAtom.ssend && !lastAtom.notshow) {
                    let endChainResi = ic.firstAtomObjCls.getLastAtomObj(
                        ic.chains[lastAtom.structure + '_' + lastAtom.chain],
                    ).resi
                ...
                if (ic.pk === 3 && bHighlight === 1 && lastAtom.ss === 'coil') {
                    let residueid =
                        lastAtom.structure + '_' + lastAtom.chain + '_' + (parseInt(lastAtom.resi) + 1).toString()
                    if (ic.residues.hasOwnProperty(residueid)) {
                        atomsAdjust = me.hashUtilsCls.unionHash(
                            atomsAdjust,
                            me.hashUtilsCls.hash2Atoms(ic.residues[residueid], ic.atoms),
