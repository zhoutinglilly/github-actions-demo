...
class Axes {
    ...
    buildAxes(radius, center, positionX, positionY, positionZ, bSelection) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
        ...
        if (bSelection) {
            ...
            meshX = ic.cylinderCls.createCylinder_base(
                new THREE.Vector3(x, y, z),
                new THREE.Vector3(x + axisLenX, y, z),
                r,
                me.parasCls.thr(0xff0000),
            ) // +X
            meshY = ic.cylinderCls.createCylinder_base(
                new THREE.Vector3(x, y, z),
                new THREE.Vector3(x, y + axisLenY, z),
                r,
                me.parasCls.thr(0x00ff00),
            ) // +Y
            meshZ = ic.cylinderCls.createCylinder_base(
                new THREE.Vector3(x, y, z),
                new THREE.Vector3(x, y, z + axisLenZ),
                r,
                me.parasCls.thr(0x0000ff),
            ) // +Z
        }
    ...
    buildAllAxes(radius, bSelection) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
    ...
    createArrow(dir, origin, axisLen, color, headLength, headWidth, bGlycan) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
