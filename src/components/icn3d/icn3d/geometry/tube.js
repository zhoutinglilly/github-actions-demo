...
class Tube {
    ...
    createTube(atoms, atomName, radius, bHighlight, bCustom, bRadiusArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
            ...
            if (atom.name === atomName && !atom.het) {
                ...
                if (
                    ...
                    (currentChain !== atom.chain ||
                        Math.abs(atom.coord.x - prevAtom.coord.x) > maxDist ||
                        Math.abs(atom.coord.y - prevAtom.coord.y) > maxDist ||
                        Math.abs(atom.coord.z - prevAtom.coord.z) > maxDist ||
                        (parseInt(currentResi) + 1 < parseInt(atom.resi) &&
                            (Math.abs(atom.coord.x - prevAtom.coord.x) > maxDist2 ||
                                Math.abs(atom.coord.y - prevAtom.coord.y) > maxDist2 ||
                                Math.abs(atom.coord.z - prevAtom.coord.z) > maxDist2)))
                ) {
                    if (bHighlight !== 2) {
                        if (!isNaN(firstAtom.resi) && !isNaN(prevAtom.resi)) {
                            let prevoneResid =
                                ...
                                firstAtom.chain +
                                '_' +
                                (parseInt(firstAtom.resi) - 1).toString()
                            ...
                            if (ic.residues.hasOwnProperty(nextoneResid)) {
                                ...
                                if (nextAtom !== undefined && nextAtom.ssbegin) {
                                    ...
                                    nexttwoResid =
                                        ...
                                        prevAtom.chain +
                                        '_' +
                                        (parseInt(prevAtom.resi) + 3).toString()
                        ...
                        pnts_colors_radii_prevone_nexttwo.push({
                            pnts: pnts,
                            colors: colors,
                            radii: radii,
                            prevone: prevone,
                ...
                if (pnts.length == 0 && !isNaN(atom.resi)) {
                    ...
                    if (ic.residues.hasOwnProperty(prevoneResid)) {
                        ...
                        if (prevAtom !== undefined && prevAtom.ssend) {
                            // include the residue
        ...
        if (bHighlight !== 2) {
            ...
            if (firstAtom !== undefined && !isNaN(firstAtom.resi)) {
                let prevoneResid =
                    firstAtom.structure + '_' + firstAtom.chain + '_' + (parseInt(firstAtom.resi) - 1).toString()
                let prevoneCoord = ic.firstAtomObjCls.getAtomCoordFromResi(prevoneResid, atomName)
                prevone = prevoneCoord !== undefined ? [prevoneCoord] : []
            }
    ...
    getCustomtubesize(resid) {
        ...
        let radiusFinal =
            ic.queryresi2score[chainid] && ic.queryresi2score[chainid].hasOwnProperty(resi)
                ? ic.queryresi2score[chainid][resi] * 0.01
                : ic.coilWidth
    ...
    // modified from iview (http://istar.cse.cuhk.edu.hk/iview/)
    createTubeSub(_pnts, colors, radii, bHighlight, prevone, nexttwo, bRadiusArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (me.bNode) return
        ...
        //        if(bRadiusArray) {
        let circleDiv = ic.tubeDIV,
            axisDiv = ic.axisDIV
        let circleDivInv = 1 / circleDiv,
            axisDivInv = 1 / axisDiv
        ...
        let prevAxis1 = new THREE.Vector3(),
            prevAxis2
        for (let i = 0, lim = pnts.length; i < lim; ++i) {
            ...
            for (let j = 0; j < circleDiv; ++j) {
                ...
                let point = pnts[i]
                    .clone()
                    .add(axis1.clone().multiplyScalar(Math.cos(angle)))
                    .add(axis2.clone().multiplyScalar(Math.sin(angle)))
                verticeArray[offset++] = point.x
                verticeArray[offset++] = point.y
                verticeArray[offset++] = point.z

                color =
                    i == colors.length - 1 && colors.length > 1
                        ? me.parasCls.thr(colors[colors.length - 2])
                        : me.parasCls.thr(colors[i])
