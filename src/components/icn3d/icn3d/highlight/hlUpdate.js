...
class HlUpdate {
    ...
    updateHlAll(commandnameArray, bSetMenu, bUnion, bForceHighlight) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    updateHlObjects(bForceHighlight) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ic.hlObjectsCls.removeHlObjects()
    ...
    updateHlSeq(bShowHighlight, residueHash, bUnion) {
        let ic = this.icn3d,
            me = ic.icn3dui
        ...
        if (ic.hAtoms && Object.keys(ic.hAtoms).length < Object.keys(ic.atoms).length)
            this.hlSequence(Object.keys(residueHash))
        this.changeSeqColor(Object.keys(residueHash))
    }

    updateHlSeqInChain(commandnameArray, bUnion) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if (bUnion === undefined || !bUnion) {
            this.removeHlSeq()
        ...
        if (chainArray2d !== undefined) {
            for (let i = 0, il = chainArray2d.length; i < il; ++i) {
                ...
                if (ic.alnChains[chainArray2d[i]] !== undefined) {
                    ...
                    if (Object.keys(alignedAtoms).length > 0)
                        firstAtom = ic.firstAtomObjCls.getFirstCalphaAtomObj(alignedAtoms)
                }
                let color =
                    firstAtom !== undefined && firstAtom.color !== undefined
                        ? '#' + firstAtom.color.getHexString()
                        : '#FFFFFF'
    ...
    updateHlMenus(commandnameArray) {
        let ic = this.icn3d,
            me = ic.icn3dui
    ...
    highlightChains(chainArray) {
        ...
        for (let c = 0, cl = chainArray.length; c < cl; ++c) {
            ...
            for (let i in ic.chainsSeq[chainid]) {
                // get residue number
    ...
    hlSummaryDomain3ddomain(that) {
        let ic = this.icn3d,
            me = ic.icn3dui
        if ($(that).attr('domain') !== undefined) {
            // domain
        ...
        if ($(that).attr('3ddomain') !== undefined) {
            // 3d domain
            let index = $(that).attr('index')
            let chainid = $(that).attr('chain')
